...
bool DireHardProcess::allowCandidates(int iPos, vector<pair<int,int> > Pos1,
...
bool DireHardProcess::findOtherCandidates(int iPos, const Event& event,
  ...
  vector<int> currentCandidates1;
  vector<int> currentCandidates2;
  // Check outgoing candidates
  for(int i=0; i < int(PosOutgoing1.size()); ++i)
    // Compare particle properties
    if ( id  == state[PosOutgoing1[i].second].id()
      ...
      currentCandidates1.push_back(i);
  // Check outgoing candidates
  for(int i=0; i < int(PosOutgoing2.size()); ++i)
    // Compare particle properties
    ...
    for(int j=0; j < int(PosOutgoing1.size()); ++j) {
      ...
      if ( state[i].isFinal()
        ...
        if ( allowCandidates(i, newPosOutgoing1, PosOutgoing2, state) ) {
          ...
          substitute1.push_back(make_pair(j, i));
        }
      }
  ...
  vector< pair<int,int> > substitute2;
  for(int i=0; i < int(state.size()); ++i)
    for(int j=0; j < int(PosOutgoing2.size()); ++j) {
      ...
      if ( state[i].isFinal()
        ...
        if ( allowCandidates(i, PosOutgoing1, newPosOutgoing2, state) ) {
          ...
          substitute2.push_back(make_pair(j, i));
        }
      }
  ...
  // Remove all hard process particles that would otherwise be counted twice.
  vector<int> iRemove;
  vector<pair<int,int> > pos1 = PosOutgoing1;
  vector<pair<int,int> > pos2 = PosOutgoing2;
  for (int i = 0; i < int(substitute2.size()); ++i) {
    bool remove = false;
    if (substitute2[i].second == iPos) remove = true;
    for(int j=0; j < int(pos2.size()); ++j)
      if (substitute2[i].second == pos2[j].second ) {
        pos2[j].second = 0; remove = true; }
    for(int j=0; j < int(pos1.size()); ++j)
      if (substitute2[i].second == pos1[j].second ) {
        pos1[j].second = 0; remove = true; }
    if ( remove ) iRemove.push_back(i);
  }
  sort (iRemove.begin(), iRemove.end());  
  for (int i = iRemove.size()-1; i >= 0; --i)
    { substitute2[iRemove[i]] = substitute2.back(); substitute2.pop_back(); }
  // Almost done - now remove double-counted particles from other potential
  // substitutions.
  iRemove.clear();
  for (int i = 0; i < int(substitute1.size()); ++i) {
    bool remove = false;
    if (substitute1[i].second == iPos) remove = true;
    for(int j=0; j < int(pos2.size()); ++j)
      if (substitute1[i].second == pos2[j].second ) {
        pos2[j].second = 0; remove = true; }
    for(int j=0; j < int(pos1.size()); ++j)
      if (substitute1[i].second == pos1[j].second ) {
        pos1[j].second = 0; remove = true; }
    if ( remove ) iRemove.push_back(i);
  }
  sort (iRemove.begin(), iRemove.end());  
  for (int i = iRemove.size()-1; i >= 0; --i)
    { substitute1[iRemove[i]] = substitute1.back(); substitute1.pop_back(); }

 // Decide if a replacment candidate has been found.
  foundCopy = (doReplace)
    ? swapCandidates(currentCandidates1, currentCandidates2,
                     substitute1,        substitute2)
    : (substitute1.size() + substitute2.size() > 0);
...
bool DireHardProcess::swapCandidates( vector<int> currentCandidates1,
    vector<int> currentCandidates2, vector< pair<int,int> > substitute1,
    vector< pair<int,int> > substitute2) {

  int nOld1 = currentCandidates1.size();
  int nOld2 = currentCandidates2.size();
  int nNew1 = substitute1.size();
  int nNew2 = substitute2.size();
  bool exchanged = false;
  // Replace, if one-to-one correspondence exists.
  if ( nOld1 == 1 && nOld2 == 0 && nNew1 == 1){
    PosOutgoing1[substitute1.front().first].second
      = substitute1.front().second;
    exchanged = true;
  } else if ( nOld1 == 0 && nOld2 == 1 && nNew2 == 1){
    PosOutgoing2[substitute2.front().first].second
      = substitute2.front().second;
    exchanged = true;
  // Else simply swap with the first candidate.
  } else if ( nOld1 > 0 && nNew1 > 0) {
    int index = 0;
    bool found = false;
    for (int i=0; i < int(substitute1.size()); ++i) {
      for (int j=0; j < int(substitute2.size()); ++j)
        if  (substitute1[i].second != substitute2[i].second) {
    ...
    //  = substitute1.front().second;
    if (found) {
      PosOutgoing1[substitute1[index].first].second
        = substitute1[index].second;
      exchanged = true;
    }
  } else if ( nOld2 > 0 && nNew2 > 0) {
    int index = 0;
    bool found = false;
    for (int i=0; i < int(substitute2.size()); ++i) {
      for (int j=0; j < int(substitute1.size()); ++j)
        if  (substitute2[i].second != substitute1[i].second) {
          index = i; found = true; break; }
      if (found) break;
    }
    if (found) {
      PosOutgoing2[substitute2[index].first].second
        = substitute2[index].second;
      exchanged = true;
    }
  }
...
bool DireHardProcess::swapCandidates( vector<int> currentCandidates1,
  ...
  // Replace, if one-to-one correspondence exists.
  if ( nOld1 == 1 && nOld2 == 0 && nNew1 == 1){
    PosOutgoing1[substitute1.front().first] = substitute1.front().second;
    exchanged = true;
  } else if ( nOld1 == 0 && nOld2 == 1 && nNew2 == 1){
    PosOutgoing2[substitute2.front().first] = substitute2.front().second;
    exchanged = true;
  // Else simply swap with the first candidate.
  } else if ( nOld1 > 0 && nNew1 > 0) {
    PosOutgoing1[substitute1.front().first] = substitute1.front().second;
...
void DireMergingHooks::init(){
  ...
  nwInc = 0;
  nhInc = 0;
  //njInc = naInc = nzInc = nwInc = nhInc = 0;
  ...
  // Initialise exact definition of kT. Only used here as auxiliary storage.
  ktTypeSave             = -99;

  // Initialise other base class switches to false.
  doMGMergingSave = doKTMergingSave = doCutBasedMergingSave
    = doNL3TreeSave = doNL3LoopSave = doNL3SubtSave = false;
  ...
  hardProcessStore.initOnProcess(processNow, particleDataPtr);
#else
  hardProcess->initOnProcess(processSave, particleDataPtr);
  hardProcessStore.initOnProcess(processSave, particleDataPtr);
#endif
  ...
  if ( doUNLOPS || doEstimateXSection ) {
    tmsValueSave    = settingsPtr->parm("Merging:TMS");
    nJetMaxSave     = settingsPtr->mode("Merging:nJetMax");
  ...
  bool writeBanner
    = doUserMergingSave || doUNLOPS || doUMEPS || doPTLundMergingSave;
...
bool DireMergingHooks::doVetoEmission( const Event& event) {
  ...
  int vetoCode = ktTypeSave;
  ktTypeSave=-99;
  if (tnow <= 0. && vetoCode!=-99 && vetoCode > 0) tnow = infoPtr->eCM();
  ...
  veto = (nSteps - 1 < nJetMax && nSteps >= 1 && tnow > tms());
  ...
  if ( getProcessString().find("inc") != string::npos )
    nSteps
      = getNumberOfClusteringSteps(
          evtUtils->eventForMerging(process, false) );
  else
    nSteps
      = (doResonance)
      ? getNumberOfClusteringSteps(process)
      : getNumberOfClusteringSteps(
          evtUtils->eventForMerging(process, false) );
  ...
  // any previous veto.
    ...
    if ( pTsave > 0. && check ) {
      ...
      for ( int i=0; i < int(goodSys.size()); ++i ) {
        ...
        double pTres = tevol(event, iRad, iEmt, iRec, 1);
...
int DireMergingHooks::getNumberOfClusteringSteps(const Event& event,
  ...
  int nFinalLeptons = 0;
  for( int i=0; i < event.size(); ++i)
    if ( event[i].isFinal() && isInHardScattering( i, event)
      && event[i].isLepton())
      nFinalLeptons++;

  // Uncolored BSM particles.
  int nBSM(0);
  for( int i=0; i < event.size(); ++i)
    if ( event[i].isFinal() && isInHardScattering( i, event)
      && event[i].colType() == 0
      && event[i].idAbs() > 1000 && !event[i].isHadron()) nBSM++;
  ...
  //int nsteps =  nFinalPartons     + nFinalLeptons     + nFinalBosons  +  nBSM
  //           - (nHardOutPartons() + nHardOutLeptons() + nHardOutBosons());
  int nsteps =  nFinal - nHardFinal();
...
bool DireMergingHooks::isFirstEmt(const Event& event ) {
...
bool DireMergingHooks::isInHardScattering( int iPos, const Event& event,
  int iSys) {
...
bool DireMergingHooks::setShowerStartingScales( bool isTrial,
  ...
  if ( isTrial ) {
    ...
    if ( limitPTmaxISR && !isInclusive ) pTmaxISR = scale;
    if ( limitPTmaxFSR && !isInclusive ) pTmaxFSR = scale;
    if ( limitPTmaxMPI && !isInclusive ) pTmaxMPI = scale;
    ...
    if ( limitPTmaxISR && isInclusive && is2to1 ) pTmaxISR = scale;
    if ( limitPTmaxFSR && isInclusive && is2to1 ) pTmaxFSR = scale;
    if ( limitPTmaxMPI && isInclusive && is2to1 ) pTmaxMPI = scale;
  ...
  if ( doMergeFirstEmm ) {
    ...
    bool doRecluster = doUMEPSSubt() || doUNLOPSSubt() || doUNLOPSSubtNLO();
...
double DireMergingHooks::tmsNow( const Event& event ) {
  ...
  tnow = (unlopsType<0) ? scalems(event) : scalept(event);
...
double DireMergingHooks::scalems( const Event& event){
  ...
  if (in1 == 0 || in2 == 0){
    ...
    for(int i=3; i < int(event.size()); ++i){
      if ( !isInHardScattering( i, event ) ) continue;
  ...
  for(int i=0; i < int(FinalPartPos.size()); ++i){
    ...
    if (event[in1].colType() != 0) {
      double temp = tevol( event, in1, FinalPartPos[i], in2, 0);
      pt12 = min(pt12, temp);
    }

    // Compute II separation of i-emission and second incoming as radiator
    if ( event[in2].colType() != 0) {
      double temp = tevol( event, in2, FinalPartPos[i], in1, 0);
      pt12 = min(pt12, temp);
    }
    ...
    if ( event[in1].colType() != 0 ) {
      for(int j=0; j < int(FinalPartPos.size()); ++j) {
        ...
        if ( i != j ){
          ...
          double temp = tevol(event,in1,FinalPartPos[i],FinalPartPos[j],0);
          pt12 = min(pt12, temp);
        }
      }
    }
    ...
    if ( event[in2].colType() != 0 ) {
      for(int j=0; j < int(FinalPartPos.size()); ++j) {
        ...
        if ( i != j ){
          // Check with first initial as recoiler
          double temp = tevol(event,in2,FinalPartPos[i],FinalPartPos[j],0);
          pt12 = min(pt12, temp);
        }
      }
    }
    ...
    for(int j=0; j < int(FinalPartPos.size()); ++j) {
      for(int k=0; k < int(FinalPartPos.size()); ++k) {
        ...
        if ( (i != j) && (i != k) && (j != k) ){
          double temp = tevol( event, FinalPartPos[i], FinalPartPos[j],
                            FinalPartPos[k], 0);
          pt12 = min(pt12, temp);
          temp        = tevol( event, FinalPartPos[j], FinalPartPos[i],
                            FinalPartPos[k], 0);
          pt12 = min(pt12, temp);
        }
      }
    ...
    if ( event[in1].colType() != 0 ) {
      for(int j=0; j < int(FinalPartPos.size()); ++j) {
        ...
        if ( i != j ){
          // Check with first initial as recoiler
          double temp = tevol( event, FinalPartPos[i],FinalPartPos[j],in1, 0);
          pt12 = min(pt12, temp);
        }
      }
    }
    ...
    if ( event[in2].colType() != 0) {
      for(int j=0; j < int(FinalPartPos.size()); ++j) {
        ...
        if ( i != j ){
          ...
          double temp = tevol( event, FinalPartPos[i],FinalPartPos[j],in2, 0);
          pt12 = min(pt12, temp);
        }
      }
    }
...
// Find the number of jets with pT > merging scale. This function can be used
// as a merging scale definition.

double DireMergingHooks::scalept( const Event& event){

  // Only consider first emissions.
  if (!isFirstEmt(event)) { setVeto(-99); return -infoPtr->eCM(); }

  double ymax=6.;
  double yPartonMax = 100.;
  int nInProcNow = nHardNowSave;
  ...
  // Run jet algorithm.
  jetAlgorithm.analyze(jetInput);
  int n(0);
  for (int i=0; i<jetAlgorithm.sizeJet(); ++i) {
    if (    jetAlgorithm.pT(i)> tms()
     && abs(jetAlgorithm.y(i))< ymax) n++;
  }

  // Get number of requested partons.
  int n_partons_in_born = nInProcNow - nRecluster();
...
double DireMergingHooks::tevol(const Event& event, int rad, int emt,
  int rec, int){
  ...
  if (isFSR) {
    vector<string> name
      = showers->timesPtr->getSplittingName(event, rad, emt, rec);
    for (int i=0; i < int(name.size()); ++i) {
      double pttemp = kinRels->tEvol(event[rad],event[emt],event[rec]);
      ptret = min(ptret,pttemp);
    }
  } else {
    vector<string> name
      = showers->spacePtr->getSplittingName(event, rad, emt, rec);
    for (int i=0; i < int(name.size()); ++i) {
      double pttemp = kinRels->tEvol(event[rad],event[emt],event[rec]);
      ptret = min(ptret,pttemp);
    }

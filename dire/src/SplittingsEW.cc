...
void DireSplittingEW::init() {
  ...
  sw2  = coupSMPtr->sin2thetaW();
  cw2  = coupSMPtr->cos2thetaW();
  m2W  = pow2(mW);
  m2Z  = pow2(mZ);
...
double DireSplittingEW::tCutoff( const Particle&, const Particle&,
  PartonSystems*) {
  return pT2min;
}
...
bool DireSplittingEW::aboveCutoff( double t, const Particle&,
  const Particle&, int, PartonSystems*) {
  if (t < pT2min) return false;
  return true;
}

//--------------------------------------------------------------------------

bool DireSplittingEW::dipoleAboveCutoff( double t, int, int, double tcut) {
...
bool Dire_fsr_ew_F2FV::canRadiate ( const Event& state, pair<int,int> ints,
...
int Dire_fsr_ew_F2FV::kinMap()                        { return 1;}
int Dire_fsr_ew_F2FV::motherID(int idDaughter)        { return idDaughter;}
int Dire_fsr_ew_F2FV::sisterID(int)                   { return 23;}
double Dire_fsr_ew_F2FV::gaugeFactor ( int, int)      { return thetaW; }
double Dire_fsr_ew_F2FV::symmetryFactor ( int, int )  { return 1.;}
int Dire_fsr_ew_F2FV::radBefID(int idRA, int)            { return idRA;}

pair<int,int> Dire_fsr_ew_F2FV::radBefCols(
  int colRadAfter, int acolRadAfter, 
  int, int) { return make_pair(colRadAfter,acolRadAfter); }

// Pick z for new splitting.
double Dire_fsr_ew_F2FV::zSplit(double zMinAbs, double, double m2dip) {
...
double Dire_fsr_ew_F2FV::overestimateInt(double zMinAbs, double,
...
double Dire_fsr_ew_F2FV::overestimateDiff(double z, double m2dip, int) {
  double wt = 0.;
  double preFac = symmetryFactor() * gaugeFactor();
...
bool Dire_fsr_ew_F2FV::calc(const Event& state, int orderNow) {
  ...
  if ( fsr->weights->hasME(fsr->makeHardEvent(0,state,true)) ) {
    ...
    if ( fsr->weights->hasME(fsr->makeHardEvent(0,trialEvent,true)) ) {
      ...
      double wtDen(0.);
      for ( map<double, DireHistory*>::iterator it = myHistory.goodBranches.begin();
      ...
      unordered_map<string, double>::iterator it = splitInfo.extras.find("unitKernel");
...
// Return true if this kernel should partake in the evolution.
bool Dire_fsr_ew_F2VF::canRadiate ( const Event& state, pair<int,int> ints,
  unordered_map<string,bool>, Settings*, PartonSystems*, BeamParticle*){
  int nFinPartons(0), nFinQ(0), nFinOther(0);
  for(int i=0; i < state.size(); ++i) {
    if (!state[i].isFinal()) continue;
...
int Dire_fsr_ew_F2VF::kinMap()                        { return 1;}
int Dire_fsr_ew_F2VF::motherID(int idDaughter)        { return idDaughter;}
int Dire_fsr_ew_F2VF::sisterID(int)                   { return 23;}
double Dire_fsr_ew_F2VF::gaugeFactor ( int, int)      { return thetaW; }
double Dire_fsr_ew_F2VF::symmetryFactor ( int, int )  { return 1.;}
int Dire_fsr_ew_F2VF::radBefID(int idRA, int)            { return idRA;}

pair<int,int> Dire_fsr_ew_F2VF::radBefCols( int colRadAfter, int acolRadAfter, 
  int, int) { return make_pair(colRadAfter,acolRadAfter); }

// Pick z for new splitting.
double Dire_fsr_ew_F2VF::zSplit(double zMinAbs, double, double m2dip) {
  double Rz = rndmPtr->flat();
  double kappa2 = pow2(settingsPtr->parm("TimeShower:pTmin"))/m2dip;
  double p = pow( 1. + pow2(1-zMinAbs)/kappa2, Rz );
  double res = 1. - sqrt( p - 1. )*sqrt(kappa2);
...
// New overestimates, z-integrated versions.
double Dire_fsr_ew_F2VF::overestimateInt(double zMinAbs, double,
  double, double m2dip, int) {
  double wt = 0.;
  double preFac = symmetryFactor() * gaugeFactor();
  // Q -> QG, soft part (currently also used for collinear part).
...
// Return overestimate for new splitting.
double Dire_fsr_ew_F2VF::overestimateDiff(double z, double m2dip, int) {
  double wt = 0.;
  double preFac = symmetryFactor() * gaugeFactor();
  double kappaOld2 = pow2(settingsPtr->parm("TimeShower:pTmin"))/m2dip;
  wt  = preFac * 2.* (1.-z) / ( pow2(1.-z) + kappaOld2);
...
// Return kernel for new splitting.
bool Dire_fsr_ew_F2VF::calc(const Event& state, int orderNow) {
...
bool Dire_fsr_ew_F2FV::canRadiate ( const Event& state, pair<int,int> ints,
  ...
  return ( state[ints.first].isFinal()
        && state[ints.first].id() == idRadBefSave
        && (state[ints.second].isQuark() || state[ints.second].isLepton()));
}

bool Dire_fsr_ew_F2FV::canRadiate (const Event& state, int iRadBef,
  int iRecBef, Settings*, PartonSystems*, BeamParticle*) {
  return ( state[iRadBef].isFinal()
        && state[iRadBef].id() == idRadBefSave
        && (state[iRecBef].isQuark() || state[iRecBef].isLepton()));
}

int Dire_fsr_ew_F2FV::kinMap()      { return 1;}
int Dire_fsr_ew_F2FV::motherID(int) { return idRadAftSave;}
int Dire_fsr_ew_F2FV::sisterID(int) { return idEmtAftSave;}

// All charged particles are potential recoilers.
int Dire_fsr_ew_F2FV::set_nCharged( const Event& state) {
  // Find charged particles.
  int nch=0;
  for (int i=0; i < state.size(); ++i) {
    if (state[i].isQuark() || state[i].isLepton()) {
...
double Dire_fsr_ew_F2FV::symmetryFactor ( int, int ) { return 1./nchSaved;}

int Dire_fsr_ew_F2FV::radBefID(int idRA, int idEA) {
  if (idRA == idRadAftSave && idEA == idEmtAftSave) return idRadBefSave;
  return 0;
}

pair<int,int> Dire_fsr_ew_F2FV::radBefCols(
  int colRadAfter, int acolRadAfter, int, int) {
  return make_pair(colRadAfter,acolRadAfter);
}

vector<pair<int,int> > Dire_fsr_ew_F2FV::radAndEmtCols(int iRad, int,
  Event state) {
  vector< pair<int,int> > ret;
  if (state[iRad].id() != idRadBefSave) return ret;
  ret = createvector<pair<int,int> >
    (make_pair(state[iRad].col(), state[iRad].acol()))
    (make_pair(0, 0));
  return ret;
}

vector <int> Dire_fsr_ew_F2FV::recPositions( const Event& state, int iRad,
  int iEmt) {

  vector<int> recs;
  if (state[iRad].id() != idRadAftSave || state[iEmt].id() != idEmtAftSave)
    return recs;
...
double Dire_fsr_ew_F2FV::overestimateInt(double zMinAbs, double,
  double, double m2dip, int) {
  // Q -> QG, soft part (currently also used for collinear part).
  double preFac    = symmetryFactor() * gaugeFactorOverest();
  double kappaMin2 = pow2(pTmin)/m2dip;
  double wt        = preFac
...
bool Dire_fsr_ew_F2FV::calc(const Event& state, int orderNow) { 
  ...
  // Now multiply with the correct coupling and add the longitudinal pieces.
  wt_base_as1 *= fV*cTV;
  wt_base_as1 += preFac*fH*cLV*0.5*(1.-z); 
  for ( unordered_map<string,double>::iterator it = wts.begin();
    it != wts.end(); ++it ) {
    it->second *= fV*cTV;
    it->second += preFac*fH*cLV*0.5*(1.-z); 
  }
...
bool Dire_fsr_ew_F2VF::canRadiate (const Event& state, int iRadBef,
  int iRecBef, Settings*, PartonSystems*, BeamParticle*) {
  return ( state[iRadBef].isFinal()
...
int Dire_fsr_ew_F2VF::kinMap()      { return 1;}
int Dire_fsr_ew_F2VF::motherID(int) { return idEmtAftSave;}
int Dire_fsr_ew_F2VF::sisterID(int) { return idRadAftSave;}

// All charged particles are potential recoilers.
int Dire_fsr_ew_F2VF::set_nCharged( const Event& state) {
  // Find charged particles.
  int nch=0;
  for (int i=0; i < state.size(); ++i) {
    if (state[i].isQuark() || state[i].isLepton()) {
...
double Dire_fsr_ew_F2VF::symmetryFactor ( int, int ) { return 1./nchSaved;}

int Dire_fsr_ew_F2VF::radBefID(int idRad, int idEmt) {
  // Note: swapped emt <-> rad
  if (idRad == idEmtAftSave && idEmt == idRadAftSave) return idRadBefSave;
  //if (idEmt == idEmtAftSave && idRad == idRadAftSave) return idRadBefSave;
  return 0;
}

pair<int,int> Dire_fsr_ew_F2VF::radBefCols(
  int, int, int colEmtAfter, int acolEmtAfter) {
  return make_pair(colEmtAfter,acolEmtAfter);
}

vector<pair<int,int> > Dire_fsr_ew_F2VF::radAndEmtCols(int iRad, int,
  Event state) {
  vector< pair<int,int> > ret;
  if (state[iRad].id() != idRadBefSave) return ret;
  ret = createvector<pair<int,int> >
...
vector <int> Dire_fsr_ew_F2VF::recPositions( const Event& state, int iRad,
  int iEmt) {
  vector<int> recs;
  if (state[iRad].id() != idEmtAftSave || state[iEmt].id() != idRadAftSave)
    return recs;
  // Particles to exclude as recoilers.
  vector<int> iExc(createvector<int>(iRad)(iEmt));
  // Find charged particles.
...
int Dire_fsr_ew_Z2QQ1::motherID(int)            { return 1;} // Use 1 as dummy variable.
int Dire_fsr_ew_Z2QQ1::sisterID(int)            { return 1;} // Use 1 as dummy variable.
...
int Dire_fsr_ew_Z2QQ2::motherID(int)            { return -1;} // Use -1 as dummy variable.
int Dire_fsr_ew_Z2QQ2::sisterID(int)            { return -1;} // Use -1 as dummy variable.
...
vector<int>Dire_fsr_ew_H2AA::recPositions(const Event& state, int iRad,
  ...
  for (int i=0; i < state.size(); ++i) {
    if ( find(iExc.begin(), iExc.end(), i) != iExc.end() ) continue;
    //if ( state[i].isCharged() || state[i].id() == 21) {
    if ( state[i].id() == 21) {
      if (state[i].isFinal())
        recs.push_back(i);
      if (state[i].mother1() == 1 && state[i].mother2() == 0)
...
bool Dire_fsr_ew_H2AA::calc(const Event& state, int orderNow) {
  ...
  // Read all splitting variables.
  double pT2(splitInfo.kinematics()->pT2);
  //double z(splitInfo.kinematics()->z); //pT2(splitInfo.kinematics()->pT2),
    //m2dip(splitInfo.kinematics()->m2Dip),
    //m2RadBef(splitInfo.kinematics()->m2RadBef),
    //m2Rad(splitInfo.kinematics()->m2RadAft),
...
//  double preFac = symmetryFactor();
//  preFac *= particleDataPtr->particleDataEntryPtr(25)->resWidthChan( 
//    sqrt(splitInfo.radBef()->m2), 22, 22);
  ...
  // Overall result.
//  double wt     = preFac * (widthNow/widthToAA) * sigBW * pT2;
...
// Return true if this kernel should partake in the evolution.
bool Dire_isr_ew_F2FV::canRadiate ( const Event& state, pair<int,int> ints,
  unordered_map<string,bool>, Settings*, PartonSystems*, BeamParticle*) {
  return ( !state[ints.first].isFinal()
        && state[ints.first].id() == idRadBefSave
        && (state[ints.second].isQuark() || state[ints.second].isLepton()));
...
bool Dire_isr_ew_F2FV::canRadiate (const Event& state, int iRadBef,
  int iRecBef, Settings*, PartonSystems*, BeamParticle*) {
  return ( !state[iRadBef].isFinal()
        && state[iRadBef].id() == idRadBefSave
        && (state[iRecBef].isQuark() || state[iRecBef].isLepton()));
...
int Dire_isr_ew_F2FV::kinMap()      { return 1;}
int Dire_isr_ew_F2FV::motherID(int) { return idRadAftSave;}
int Dire_isr_ew_F2FV::sisterID(int) { return idEmtAftSave;}

// All charged particles are potential recoilers.
int Dire_isr_ew_F2FV::set_nCharged( const Event& state) {
  // Find charged particles.
  int nch=0;
  for (int i=0; i < state.size(); ++i) {
    if (state[i].isQuark() || state[i].isLepton()) {
...
double Dire_isr_ew_F2FV::symmetryFactor ( int, int ) { return 1./nchSaved;}

int Dire_isr_ew_F2FV::radBefID(int idRA, int) {
  if (idRA == idRadAftSave) return idRadBefSave;
  return 0;
}

pair<int,int> Dire_isr_ew_F2FV::radBefCols(
  int colRadAfter, int acolRadAfter, int, int) {
  return make_pair(colRadAfter,acolRadAfter);
}

vector<pair<int,int> > Dire_isr_ew_F2FV::radAndEmtCols(int iRad, int,
  Event state) {
  vector< pair<int,int> > ret;
  if (state[iRad].id() != idRadBefSave) return ret;
  ret = createvector<pair<int,int> >
...
vector <int> Dire_isr_ew_F2FV::recPositions( const Event& state, int iRad,
  int iEmt) {
...
// Pick z for new splitting.
double Dire_isr_ew_F2FV::zSplit(double zMinAbs, double, double m2dip) {
  double Rz        = rndmPtr->flat();
  double kappaMin2 = pow2(pTmin)/m2dip;
  double p         = pow( 1. + pow2(1-zMinAbs)/kappaMin2, Rz );
  double res       = 1. - sqrt( p - 1. )*sqrt(kappaMin2);
...
// New overestimates, z-integrated versions.
double Dire_isr_ew_F2FV::overestimateInt(double zMinAbs, double,
  double, double m2dip, int) {
  // Q -> QG, soft part (currently also used for collinear part).
  double preFac    = symmetryFactor() * gaugeFactorOverest();
  double kappaMin2 = pow2(pTmin)/m2dip;
...
// Return overestimate for new splitting.
double Dire_isr_ew_F2FV::overestimateDiff(double z, double m2dip, int) {
  double preFac    = symmetryFactor() * gaugeFactorOverest();
  double kappaMin2 = pow2(pTmin)/m2dip;
  double wt        = preFac
                     *2. * (1.-z) / ( pow2(1.-z) + kappaMin2);
...
// Return kernel for new splitting.
bool Dire_isr_ew_F2FV::calc(const Event& state, int orderNow) { 
  ...
  // Correction for massive splittings.
  bool doMassive = ((m2Rec>0. || m2Emt>0.) && splitType == 2);
  //bool doMassive = (abs(splitType) == 2);
  ...
  if (doMassive && order >= 0) {
    ...
    // Mass correction for recoiler.
    //double massCorr = -2. * m2Rec / m2dip * uCS / (1.-uCS);
    // Mass correction for emission.
    double q2 = m2dip + m2RadBef + m2Rec;
    double massCorr = -sqrt(pow2(q2-m2Rec-m2Emt)/lABC(q2,m2Rad,m2Rec))*(1.+z);

...
void DireSplittingQCD::init() {
  ...
  // Set up alphaS
  pTmin              = (is_fsr)
                     ? settingsPtr->parm("TimeShower:pTmin")
                     : settingsPtr->parm("SpaceShower:pTmin");
...
double DireSplittingQCD::tCutoff( const Particle&, const Particle&,
  PartonSystems*) {
  return pow2(pTmin);
}
...
bool DireSplittingQCD::aboveCutoff( double t, const Particle&,
  const Particle&, int, PartonSystems*) {
  if (t < pow2(pTmin)) return false;
  return true;
}

//--------------------------------------------------------------------------

bool DireSplittingQCD::dipoleAboveCutoff( double t, int, int, double tcut) {
...
double DireSplittingQCD::couplingScale2 (double z, double pT2,
  double m2dip, pair<int,bool> radBef, pair<int,bool> recBef) {
  if        ( radBef.second &&  recBef.second) {
    ...
    if (asSchemeFSR == 2) {
      ...
      double kt2 = sij*sjk/sik;
      if (std::isnan(kt2)) return pT2;
      return kt2;
    }
  } else if ( radBef.second && !recBef.second) {
    if (asSchemeFSR == 0) return pT2;
    if (asSchemeFSR == 1) {
      // CS evolution variable already collinear kT.
      if (settingsPtr->mode("DireTimes:evolutionMode") !=0) return pT2;
      double zcs = z;
    ...
    if (asSchemeFSR == 2) {
      double zcs = z;
      double xcs = m2dip * zcs * (1.-zcs) / (pT2 + m2dip * zcs * (1.-zcs));
      if (settingsPtr->mode("DireTimes:evolutionMode") !=0)
        xcs = fsr->kinRels->xcs_fi(z,pT2,m2dip);
      double kt2 = (1-zcs)*(1-xcs)/xcs/zcs*m2dip;
      if (std::isnan(kt2)) return pT2;
      return kt2;
    }
  } else if (!radBef.second &&  recBef.second) {
    ...
    if (asSchemeISR == 2) {
      double xcs = z;
      double ucs = pT2/m2dip / (1.-z);
      if ( settingsPtr->mode("DireSpace:evolutionMode") !=0)
        ucs = isr->kinRels->ucs_if(z,pT2,m2dip);
      double kt2 = (1-xcs)/xcs*ucs/(1-ucs)*m2dip;
      if (std::isnan(kt2)) return pT2;
      return kt2;
    }
  } else if (!radBef.second && !recBef.second) {
...
unordered_map<string,double> DireSplittingQCD::getPhasespaceVars( const Event& state,
  PartonSystems*) {
...
double DireSplittingQCD::getJacobian( const Event& state, PartonSystems*
  ...
  if ( splitInfo.radBef()->isFinal && splitInfo.recBef()->isFinal ) {
    ...
    //if (m2RadBef>m2Rad+m2Emt) { cout << "FFjac " << name() << " " << yCS*Q2 << " " << jacobian << endl; jacobian=1.;}

    /*double fac=10.;
    double m2 = yCS*Q2;
    if ( m2RadBef>m2Rad+m2Emt
      && jacobian > 100.
      && (m2 > m2RadBef*fac || m2 < m2RadBef/fac))
      jacobian=1.;
    if ( splitInfo.radBef()->id < 1000 && !is_qcd && !is_qed && !is_ewk) { jacobian=1.;}*/

    //jacobian = min(jacobian,50.);
  ...
  // Final-initial jacobian
    ...
    // the Jacobian.
    int iOther = (splitInfo.iRecBef == 3 ? 4 : 3);
    if (hasSys) iOther = (state[splitInfo.iRecBef].mother1() == 1)
    ...
    if ( nEmissions() != 2 ) {
      ...
      //jacobian = min(jacobian,50.);
      //if (m2RadBef>m2Rad+m2Emt) jacobian=1.;
//      if (m2RadBef>m2Rad+m2Emt) { 
//double sij  = (1.-1./xCS) * q2 + m2RadBef/xCS;
//cout << "FIjac " << name() << " " << sij << " " << jacobian << endl; jacobian=1.;}
    }
...
vector<pair<int,int> > Dire_fsr_qcd_Q2QGG::radAndEmtCols(int iRad, int colType,
  ...
  if ( colType > 0) {
    ...
    splitInfo.colEmtInt =  newCol1;
    splitInfo.acolEmtInt = state[iRad].acol();
    splitInfo.colRadInt =  state[iRad].col();
    splitInfo.acolRadInt = newCol1;
  } else {
    ...
    splitInfo.colEmtInt =  state[iRad].col();
    splitInfo.acolEmtInt = newCol1;
    splitInfo.colRadInt =  newCol1;
...
double Dire_fsr_qcd_Q2QGG::zSplit(double zMinAbs, double zMaxAbs,
  double m2dip) {
  double R = rndmPtr->flat();
  // Pick according to soft + 1/(z+kappa2)
  double kappa2 = pow2(pTmin)/m2dip;
  double z1 = pow((1+kappa2)/kappa2,-R)*(1+kappa2) - kappa2;
  if (kappa2 < 1e-5) z1 = pow(kappa2,0.5)
        * tan(   R*atan(zMaxAbs*pow(kappa2,-0.5))
           -(R-1.)*atan(zMinAbs*pow(kappa2,-0.5)));
  return z1;
}

// New overestimates, z-integrated versions.
double Dire_fsr_qcd_Q2QGG::overestimateInt(double zMinAbs, double zMaxAbs,
  ...
  if (kappa2 < 1e-5)
    wt = 16.*CF * ( atan(zMaxAbs*pow(kappa2,-0.5))
                  - atan(zMinAbs*pow(kappa2,-0.5)))*pow(kappa2,-0.5);
  return wt;
}
...
double Dire_fsr_qcd_Q2QGG::overestimateDiff(double z, double m2dip, int) {
  ...
  // Overestimate by soft + 1/(z+kappa2)
  wt  = 16*CF / (z + kappa2);
  if (kappa2 < 1e-5) wt = 16.*CF / (z*z + kappa2);
  return wt;
}
...
double Dire_fsr_qcd_Q2QGG::counterTerm(double si1, double si2, double sj1,
  ...
  if ( kTi12 > kT12 && kTi1j > pow2(pTmin)) return 0.;
...
double Dire_fsr_qcd_G2GGG::zSplit(double zMinAbs, double, double m2dip) {
  double R = rndmPtr->flat();
  // Pick according to soft + 1/(z+kappa2)
  double a = pow(pow2(pTmin)/m2dip,1.);
  double zmin = zMinAbs;
...
double Dire_fsr_qcd_G2GGG::overestimateInt(double zMinAbs, double zMaxAbs,
  ...
  //int order     = (orderNow > 0) ? orderNow : correctionOrder;
  int order     = (orderNow > -1) ? orderNow : correctionOrder;
  double kappa2 = pow(pow2(pTmin)/m2dip,1.);
  // Overestimate by soft + 1/(z+kappa2)
...
double Dire_fsr_qcd_G2GGG::overestimateDiff(double z, double m2dip,
  ...
  //int order        = (orderNow > 0) ? orderNow : correctionOrder;
  int order        = (orderNow > -1) ? orderNow : correctionOrder;
  double kappaOld2 = pow(pow2(pTmin)/m2dip,1.);
  // Overestimate by soft + 1/(z+kappa2)
...
// Pick z for new splitting.
double Dire_fsr_qcd_G2GGG::zSplit(double zMinAbs, double zMaxAbs,
  double m2dip) {
  double R = rndmPtr->flat();
  // Pick according to soft + 1/(z+kappa2)
  double kappa2 = pow2(pTmin)/m2dip;
...
double Dire_fsr_qcd_G2GGG::overestimateInt(double zMinAbs, double zMaxAbs,
  ...
  double kappa2 = pow2(pTmin)/m2dip;
  // Overestimate by soft + 1/(z+kappa2)
  wt = 8*CA/2.*CA*log( (kappa2 + 1)/kappa2);
  if (kappa2 < 1e-5)
    wt = 8.*CA/2.*CA * ( atan(zMaxAbs*pow(kappa2,-0.5))
                  - atan(zMinAbs*pow(kappa2,-0.5)))*pow(kappa2,-0.5);
  return wt;
}

// Return overestimate for new splitting.
double Dire_fsr_qcd_G2GGG::overestimateDiff(double z, double m2dip, int) {
  double wt     = 0.;
  double kappa2 = pow2(pTmin)/m2dip;
  // Overestimate by soft + 1/(z+kappa2)
  wt  = 8.*CA/2.*CA / (z + kappa2);
  if (kappa2 < 1e-5) wt = 8.*CA/2.*CA / (z*z + kappa2);
  return wt;
}
...
double Dire_fsr_qcd_G2GGG::counterTerm(double si1, double si2, double sj1,
  ...
  // Full counter-term
  double ct = 2.0 * ((gg1Soft+gg1Coll+gg1ColorCorr) / si1 +(gg2Soft+gg2Coll) / s12)
                  * sij / ((si1+si2)*(sj1+sj2)-sij*s12);
...
// Return kernel for new splitting.
//bool Dire_fsr_qcd_G2GGG::calc(const Event& state, int) { 
...
vector<pair<int,int> > Dire_fsr_qcd_Q2Qqqbar::radAndEmtCols(int iRad, int colType,
  Event state) { 
...
// Pick z for new splitting.
double Dire_fsr_qcd_Q2Qqqbar::zSplit(double zMinAbs, double zMaxAbs,
  double m2dip) {
  /*double R = rndmPtr->flat();
  // Pick according to soft + 1/(z+kappa2)
  double a = pow(pow2(pTmin)/m2dip,1.);
  ...
  double kappaMin2 = pow2(pTmin)/m2dip;
  double pMin      = 1. + pow2(1-zMinAbs)/kappaMin2;
  double pMax      = 1. + pow2(1-zMaxAbs)/kappaMin2;
  double res       = 1. - sqrt( pow(pMin,Rz)*pow(pMax,1-Rz) - 1.)
                         *sqrt(kappaMin2);
  return res;
}
...
double Dire_fsr_qcd_Q2Qqqbar::overestimateInt(double zMinAbs, double zMaxAbs,
  ...
  //             + 2.*log( (kappa2+zMaxAbs)/(kappa2+zMinAbs)) );
  wt = CF*TR * 2.* 0.5
     * log( (1. + pow2(1.-zMinAbs)/kappa2)
           /(1. + pow2(1.-zMaxAbs)/kappa2));
  return wt;
}
...
double Dire_fsr_qcd_Q2Qqqbar::overestimateDiff(double z, double m2dip,
  ...
  wt  = CF*TR * 2.* (1.-z) / ( pow2(1.-z) + kappa2);
  return wt;
}
...
bool Dire_fsr_qcd_Q2Qqqbar::calc(const Event& state, int orderNow) { 
  ...
  // Spectrum.
    ...
    // never produced by PS, hence no underlying Born to correct.
    double kT12 = ((si1+si2)*(sj1+sj2)-sij*s12)/(si1+si2+sj1+sj2+sij+s12);
    if ( abs(sai) < 1e-10 || kT12 < pow2(pTmin) ) {
      storeKernelVal("base", 0.);
      if (doVariations && settingsPtr->parm("Variations:muRfsrDown") != 1.)
        storeKernelVal("Variations:muRfsrDown", 0.);
      if (doVariations && settingsPtr->parm("Variations:muRfsrUp")   != 1.)
...
vector<pair<int,int> > Dire_fsr_qcd_G2Gqqbar::radAndEmtCols(int iRad, int colType,
  Event state) { 
...
// Pick z for new splitting.
double Dire_fsr_qcd_G2Gqqbar::zSplit(double zMinAbs, double zMaxAbs,
  double m2dip) {
  double Rz        = rndmPtr->flat();
  double kappaMin2 = pow2(pTmin)/m2dip;
  double pMin      = 1. + pow2(1-zMinAbs)/kappaMin2;
...
double Dire_fsr_qcd_G2Gqqbar::overestimateInt(double zMinAbs, double zMaxAbs,
  ...
  wt = CA/2.*TR*2.* 0.5*log( (1. + pow2(1.-zMinAbs)/kappa2)
                            /(1. + pow2(1.-zMaxAbs)/kappa2));
  return wt;
}

// Return overestimate for new splitting.
double Dire_fsr_qcd_G2Gqqbar::overestimateDiff(double z, double m2dip, int) {
  double wt        = 0.;
  double kappa2    = pow(pow2(pTmin)/m2dip,1.);
  // Overestimate by soft.
  wt  = CA/2.*TR*2.*(1.-z) / ( pow2(1.-z) + kappa2);
  return wt;
}
...
int Dire_fsr_qcd_Q2QG::radBefID(int idRA, int idEA) {
  if (particleDataPtr->isQuark(idRA) && idEA == 21) return idRA;
  return 0;
}
...
vector <int> Dire_fsr_qcd_Q2QG::recPositions( const Event& state, int iRad, int iEmt) {

  if ( !state[iRad].isQuark() || !state[iEmt].isGluon()
    || !hasSharedColor(state, iRad, iEmt) )
    return vector<int>();
...
// Return overestimate for new splitting.
double Dire_fsr_qcd_Q2QG::overestimateDiff(double z, double m2dip, int orderNow) {
  double preFac    = symmetryFactor() * gaugeFactor();
  int order        = (orderNow > -1) ? orderNow : correctionOrder;
  double kappaMin2 = pow2(pTmin)/m2dip;
  double wt        = preFac * softRescaleInt(order)
...
bool Dire_fsr_qcd_Q2QG::calc(const Event& state, int orderNow) { 
  ...
  double preFac = symmetryFactor() * gaugeFactor();
  int order     = (orderNow > -1) ? orderNow : correctionOrder;
  //double kappa2 = max(pow2(pTmin)/m2dip, pT2/m2dip);
  ...
  // emitted a soft gluon.
    ...
    if (doVariations) {
      ...
      if (settingsPtr->parm("Variations:muRfsrDown") != 1.)
        ...
        softRescaleDiff( order, scale2, (scale2 > pT2minVariations)
          ? settingsPtr->parm("Variations:muRfsrDown")*renormMultFac : renormMultFac)*fullSoft
        + fullColl + colorCorrection));
      if (settingsPtr->parm("Variations:muRfsrUp")   != 1.)
        wts.insert( make_pair("Variations:muRfsrUp",
        softRescaleDiff( order, scale2, (scale2 > pT2minVariations)
          ? settingsPtr->parm("Variations:muRfsrUp")*renormMultFac : renormMultFac)*fullSoft
        + fullColl + colorCorrection));
    }
...
//      it->second += preFac * massCorr;
    ...
    if (doGeneralizedKernel) {
      ...
      //massCorr *= as2Pi(pT2/(1.-z)) / as2Pi(pT2);
    } else {
...
vector <int> Dire_fsr_qcd_Q2GQ::recPositions( const Event& state, int iRad, int iEmt) {
  ...
  //if (!(state[iRad].idAbs() < 20 && state[iEmt].id() == 21))
  //  return vector<int>();
...
// Return overestimate for new splitting.
double Dire_fsr_qcd_Q2GQ::overestimateDiff(double z, double m2dip, int orderNow) {
  double preFac    = symmetryFactor() * gaugeFactor();
  int order        = (orderNow > -1) ? orderNow : correctionOrder;
  double kappaMin2 = pow2(pTmin)/m2dip;
...
int Dire_fsr_qcd_G2GG1::radBefID(int idRA, int idEA) { 
  if (idRA == 21 && idEA == 21) return 21;
  return 0;
}
...
vector <int> Dire_fsr_qcd_G2GG1::recPositions( const Event& state, int iRad, int iEmt) {

  //if (!(state[iRad].idAbs() == 21 && state[iEmt].id() == 21))
  //  return vector<int>();

  if ( !state[iRad].isGluon() || !state[iEmt].isGluon()
    || !hasSharedColor(state, iRad, iEmt) )
    return vector<int>();
...
// Return overestimate for new splitting.
double Dire_fsr_qcd_G2GG1::overestimateDiff(double z, double m2dip, int orderNow) {
  // Overestimate by soft
  double preFac    = symmetryFactor() * gaugeFactor();
  //int order        = (orderNow > 0) ? orderNow : correctionOrder;
  int order        = (orderNow > -1) ? orderNow : correctionOrder;
...
bool Dire_fsr_qcd_G2GG1::calc(const Event& state, int orderNow) {
  ...
  // Corrections for correlated splittings.
  bool doMultiPole = (doCorrelations && direInfoPtr->isSoft(splitInfo.iRadBef));
  ...
  if (!doMultiPole) {
    ...
    // Add collinear term for massless splittings.
    //double coll = (!doMassive && order >= 0)
    //              ? preFac * ( -1. + 0.5 * z*(1.-z) ) : 0.;
    // Add collinear term for massless splittings.
    double coll = 0.;
    if (!doMassive && order >= 0) {
      if (doGeneralizedKernel) {
...
int Dire_fsr_qcd_G2GG2::radBefID(int idRA, int idEA){
  if (idRA == 21 && idEA == 21) return 21;
  return 0;
}
...
vector <int> Dire_fsr_qcd_G2GG2::recPositions( const Event& state, int iRad, int iEmt) {
...
// Return overestimate for new splitting.
double Dire_fsr_qcd_G2GG2::overestimateDiff(double z, double m2dip, int orderNow) {
  // Overestimate by soft
  double preFac    = symmetryFactor() * gaugeFactor();
  //int order        = (orderNow > 0) ? orderNow : correctionOrder;
  int order        = (orderNow > -1) ? orderNow : correctionOrder;
...
// Return true if this kernel should partake in the evolution.
bool Dire_fsr_qcd_G2FF::canRadiate ( const Event& state, pair<int,int> ints,
  unordered_map<string,bool>, Settings*, PartonSystems*, BeamParticle*) {
  return ( state[ints.first].isFinal()
        && state[ints.second].colType() != 0
        && hasSharedColor(state, ints.first, ints.second)
...
bool Dire_fsr_qcd_G2FF::canRadiate (const Event& state, int iRadBef,
  int iRecBef, Settings*, PartonSystems*, BeamParticle*) {
  return ( state[iRadBef].isFinal()
        && state[iRecBef].colType() != 0
        && hasSharedColor(state, iRadBef, iRecBef)
...
pair<int,int> Dire_fsr_qcd_G2FF::radBefCols(
  int colRadAfter, int acolRadAfter, 
  int colEmtAfter, int acolEmtAfter) {
...
vector<pair<int,int> > Dire_fsr_qcd_G2FF::radAndEmtCols(int iRad,
  int, Event state) {
  vector< pair<int,int> > ret;
  if (state[iRad].id() != 21) return ret;

  int colRadAft(0), acolRadAft(0), colEmtAft1(0), acolEmtAft1(0);

  if (idRadAfterSave > 0) {
    colRadAft   = state[iRad].col();
    acolRadAft  = 0;
    colEmtAft1  = 0;
    acolEmtAft1 = state[iRad].acol();
  } else {
    acolRadAft   = state[iRad].acol();
    colRadAft  = 0;
    acolEmtAft1  = 0;
    colEmtAft1 = state[iRad].col();
  }
...
// Pick z for new splitting.
double Dire_fsr_qcd_G2FF::zSplit(double zMinAbs, double zMaxAbs,
  double /*m2dip*/) {
  return (zMinAbs + rndmPtr->flat() * (zMaxAbs - zMinAbs));
}

// New overestimates, z-integrated versions.
double Dire_fsr_qcd_G2FF::overestimateInt(double zMinAbs,double zMaxAbs,
...
// Return kernel for new splitting.
double Dire_fsr_qcd_G2FF::overestimateDiff(double /*z*/, double /*m2dip*/,
  int /*order*/) {
  double preFac = symmetryFactor() * abs(gaugeFactor());
...
bool Dire_fsr_qcd_G2FF::calc(const Event& state, int orderNow) {
...
int Dire_fsr_qcd_G2QQ1::radBefID(int idRA, int idEA){
  if ( particleDataPtr->isQuark(idRA)
    && particleDataPtr->isQuark(idEA)
    && idRA == -idEA) return 21;
  return 0;
}
...
vector <int> Dire_fsr_qcd_G2QQ1::recPositions( const Event& state, int iRad, int iEmt) {
  ...
  // Uncorrelated g->qq splitting.
    ...
    if (doGeneralizedKernel) {
      wt_base_as1 = preFac * ( pow(1.-z,2.) + pow(z,2.) )
        ...
        * exp(1./z*cExp(-1)  + cExp(0)  + z*cExp(1)  + z*z*cExp(2))
        + preFac * fCoef();
      //wt_base_as1 *= as2Pi(pT2/(1.-z)) / as2Pi(pT2);
    } else {
      wt_base_as1 = preFac * ( pow(1.-z,2.) + pow(z,2.) );
    }
  }
...
int Dire_fsr_qcd_G2QQ2::radBefID(int idRA, int idEA){
  if ( particleDataPtr->isQuark(idRA)
    && particleDataPtr->isQuark(idEA)
    && idRA == -idEA) return 21;
  return 0;
...
vector <int> Dire_fsr_qcd_G2QQ2::recPositions( const Event& state, int iRad, int iEmt) {
...
double Dire_fsr_qcd_Q2qQqbarDist::zSplit(double zMinAbs, double zMaxAbs,
  ...
  double Rz         = rndmPtr->flat();
  double kappa4  = pow(pTmin, 4) / pow2(m2dip);
  double res     = 1.;
...
// New overestimates, z-integrated versions.
double Dire_fsr_qcd_Q2qQqbarDist::overestimateInt(double zMinAbs, double zMaxAbs,
  double, double m2dip, int orderNow) {

  // Do nothing without other NLO kernels!
  //int order     = (orderNow > 0) ? orderNow : correctionOrder;
  int order     = (orderNow > -1) ? orderNow : correctionOrder;
  if (order != 3 && order !=5) return 0.0;

  double preFac  = symmetryFactor() * gaugeFactor();
  double pT2min  = pow2(pTmin);
...
// Return true if this kernel should partake in the evolution.
bool Dire_fsr_qcd_Q2QbarQQId::canRadiate ( const Event& state, pair<int,int> ints,
  unordered_map<string,bool>, Settings*, PartonSystems*, BeamParticle*) {
  return ( state[ints.first].isFinal()
        && state[ints.second].colType() != 0
        && hasSharedColor(state, ints.first, ints.second)
...
int Dire_isr_qcd_Q2QG::radBefID(int idRA, int idEA) { 
  if (particleDataPtr->isQuark(idRA) && idEA == 21) return idRA;
  return 0;
  //return idRA;
}
...
vector <int> Dire_isr_qcd_Q2QG::recPositions( const Event& state, int iRad, int iEmt) {
...
// Return overestimate for new splitting.
double Dire_isr_qcd_Q2QG::overestimateDiff(double z, double m2dip, int orderNow) {
  double wt        = 0.;
  double preFac    = symmetryFactor() * gaugeFactor();
  //int order        = (orderNow > 0) ? orderNow : correctionOrder;
  int order        = (orderNow > -1) ? orderNow : correctionOrder;
...
bool Dire_isr_qcd_Q2QG::calc(const Event& state, int orderNow) {
  ...
  //int order     = (orderNow > 0) ? orderNow : correctionOrder;
  int order     = (orderNow > -1) ? orderNow : correctionOrder;
  //double kappa2 = max(pow2(settingsPtr->parm("SpaceShower:pTmin"))/m2dip, pT2/m2dip);
  //double kappa2 = max(pow2(pTmin)/m2dip, pT2/m2dip);
  double kappa2 = tDire/m2dip;
...
int Dire_isr_qcd_G2GG1::radBefID(int idRA, int idEA){
  if (idRA == 21 && idEA == 21) return 21;
  return 0;
}
...
vector <int> Dire_isr_qcd_G2GG1::recPositions( const Event& state, int iRad, int iEmt) {
...
// Return overestimate for new splitting.
double Dire_isr_qcd_G2GG1::overestimateDiff(double z, double m2dip, int orderNow) {
  double wt        = 0.;
  double preFac    = symmetryFactor() * gaugeFactor();
  //int order        = (orderNow > 0) ? orderNow : correctionOrder;
  int order        = (orderNow > -1) ? orderNow : correctionOrder;
...
bool Dire_isr_qcd_G2GG1::calc(const Event& state, int orderNow) {
  ...
  if (!doMassive && order == 3) {
    for ( unordered_map<string,double>::iterator it = wts.begin(); it !=wts.end(); ++it){
      ...
      // colour structures, and hence should contribute equally to Dire_isr_qcd_G2GG1
      // and Dire_isr_qcd_G2GG2. Hence one factor of 0.5 . Then, another factor of 0.5
...
int Dire_isr_qcd_G2GG2::radBefID(int idRA, int idEA){
 if (idRA==21 && idEA == 21) return 21;
 return 0;
}
...
vector <int> Dire_isr_qcd_G2GG2::recPositions( const Event& state, int iRad, int iEmt) {
...
int Dire_isr_qcd_G2QQ::radBefID(int idRA, int idEA){ 
  if (idRA == 21 && particleDataPtr->isQuark(idEA)) return -idEA;
  return 0;
  //return -idEA;
}
...
vector <int> Dire_isr_qcd_G2QQ::recPositions( const Event& state, int iRad, int iEmt) {

  if ( !state[iRad].isGluon() || !state[iEmt].isQuark() 
    || !hasSharedColor(state, iRad, iEmt) )
    return vector<int>();
...
int Dire_isr_qcd_Q2GQ::radBefID(int idRA, int idEA){
  if ( particleDataPtr->isQuark(idRA) && particleDataPtr->isQuark(idEA)
    && idRA == idEA) return 21;
  return 0;
}
...
vector <int> Dire_isr_qcd_Q2GQ::recPositions( const Event& state, int iRad, int iEmt) {
...
// New overestimates, z-integrated versions.
double Dire_isr_qcd_Q2qQqbarDist::overestimateInt(double zMinAbs, double zMaxAbs,
  double, double m2dip, int orderNow) {
...
// Return true if this kernel should partake in the evolution.
bool Dire_isr_qcd_Q2QbarQQId::canRadiate ( const Event& state, pair<int,int> ints,
  unordered_map<string,bool>, Settings*, PartonSystems*, BeamParticle*) {
  return (!state[ints.first].isFinal()
        && state[ints.second].colType() != 0
        && hasSharedColor(state, ints.first, ints.second)
...
// Return true if this kernel should partake in the evolution.
bool Dire_fsr_qcd_Q2QG_notPartial::canRadiate ( const Event& state, pair<int,int> ints,
  unordered_map<string,bool>, Settings*, PartonSystems*, BeamParticle*) {
  return ( state[ints.first].isFinal()
        && state[ints.second].colType() == 0
        && state[ints.first].isQuark() );
...
int Dire_fsr_qcd_Q2QG_notPartial::radBefID(int idRad, int idEmt) {
  if (particleDataPtr->isQuark(idRad) && idEmt==21) return idRad;
  return 0;
}
...
// Pick z for new splitting.
double Dire_fsr_qcd_Q2QG_notPartial::zSplit(double zMinAbs, double, double m2dip) {
  double Rz        = rndmPtr->flat();
...
// Return overestimate for new splitting.
double Dire_fsr_qcd_Q2QG_notPartial::overestimateDiff(double z, double m2dip, int) {
...
bool Dire_fsr_qcd_Q2QG_notPartial::calc(const Event& state, int) { 
  ...
  // Note: We are calculating the z <--> 1-z symmetrised kernel here.
  double preFac = symmetryFactor() * gaugeFactor();
  //double kappa2 = max(pow2(settingsPtr->parm("TimeShower:pTmin"))/m2dip, pT2/m2dip);
  //double kappa2 = max(pow2(pTmin)/m2dip, pT2/m2dip);
  double kappa2 = tDire/m2dip;
...
// Return true if this kernel should partake in the evolution.
bool Dire_fsr_qcd_G2GG_notPartial::canRadiate ( const Event& state, pair<int,int> ints,
  unordered_map<string,bool>, Settings*, PartonSystems*, BeamParticle*) {
  return ( state[ints.first].isFinal()
        && state[ints.second].colType() == 0
        && state[ints.first].id() == 21 );
...
//double Dire_fsr_qcd_G2GG_notPartial::symmetryFactor ( int, int )     { return 1.0;}

int Dire_fsr_qcd_G2GG_notPartial::radBefID(int idRad, int idEmt) {
  if (idRad==21 && idEmt==21) return idRad;
  return 0;
}
...
vector<pair<int,int> > Dire_fsr_qcd_G2GG_notPartial::radAndEmtCols(int iRad,
  ...
  vector< pair<int,int> > ret;
  if (!state[iRad].isGluon() || state[splitInfo.iRecBef].colType() != 0)
    return ret;
...
vector <int> Dire_fsr_qcd_G2GG_notPartial::recPositions( const Event&, int, int) {
  return vector <int>();
}

// Pick z for new splitting.
double Dire_fsr_qcd_G2GG_notPartial::zSplit(double zMinAbs, double, double m2dip) {
  // Just pick according to soft.
  double R         = rndmPtr->flat();
  double kappaMin4 = pow4(settingsPtr->parm("TimeShower:pTmin"))/pow2(m2dip);
...
// Return overestimate for new splitting.
double Dire_fsr_qcd_G2GG_notPartial::overestimateDiff(double z, double m2dip, int) {
  // Overestimate by soft
  double preFac    = symmetryFactor() * gaugeFactor();
  double kappaMin4 = pow4(settingsPtr->parm("TimeShower:pTmin"))/pow2(m2dip);
...
// Return true if this kernel should partake in the evolution.
bool Dire_fsr_qcd_G2QQ_notPartial::canRadiate ( const Event& state, pair<int,int> ints,
  unordered_map<string,bool>, Settings*, PartonSystems*, BeamParticle*) {
  return ( state[ints.first].isFinal()
        && state[ints.second].colType() == 0
        && state[ints.first].id() == 21 );
...
int Dire_fsr_qcd_G2QQ_notPartial::motherID(int)            { return 1;} // Use 1 as dummy variable.
int Dire_fsr_qcd_G2QQ_notPartial::sisterID(int)            { return 1;} // Use 1 as dummy variable.
double Dire_fsr_qcd_G2QQ_notPartial::gaugeFactor ( int, int )        { return 2.*NF_qcd_fsr*TR;}
double Dire_fsr_qcd_G2QQ_notPartial::symmetryFactor ( int, int )     { return 0.5;}

int Dire_fsr_qcd_G2QQ_notPartial::radBefID(int idRad, int idEmt) {
  if (particleDataPtr->isQuark(idRad) && idRad == -idEmt) return 21;
  return 0;
}
...
vector <int> Dire_fsr_qcd_G2QQ_notPartial::recPositions( const Event&, int, int) {
  return vector <int>();
}

// Pick z for new splitting.
double Dire_fsr_qcd_G2QQ_notPartial::zSplit(double zMinAbs, double zMaxAbs, double) {
  return (zMinAbs + rndmPtr->flat() * (zMaxAbs - zMinAbs));
}

// New overestimates, z-integrated versions.
double Dire_fsr_qcd_G2QQ_notPartial::overestimateInt(double zMinAbs,double zMaxAbs,
  double, double, int) {
  double wt     = 0.;
  double preFac = symmetryFactor() * gaugeFactor();
  wt            = 2.*preFac * 0.5 * ( zMaxAbs - zMinAbs);

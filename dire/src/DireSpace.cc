...
//const string DireSpace::testname="Dire_isr_qed_11->11&22";
//const string DireSpace::testps="II";
//const string DireSpace::testname="Dire_isr_u1new_11->11&22";
//const string DireSpace::testps="II";
//const string DireSpace::testname="Dire_isr_qed_11->11&22";
//const string DireSpace::testps="IF";
//const string DireSpace::testname="Dire_isr_u1new_11->11&22";
//const string DireSpace::testps="IF";

const string DireSpace::testname="N/A";
const string DireSpace::testps="N/A";
...
void DireSpace::init( BeamParticle* beamAPtrIn,
  ...
  double pT2minQED = pow2(settingsPtr->parm("DireSpace:QED:pTminChgQ"));
  ...
  // Default value for SpaceShower-internal settings to avoid illdefined
  // behavior.
  //rescatterFail = false;
  //gamma2qqbar   = false;
...
bool DireSpace::appendAllowedEmissions( const Event& state, DireSpaceEnd* dip) {
  ...
  /*for ( unordered_map<string,DireSplitting*>::iterator it = splits.begin();
    ...
    for (int iEmtAft=1; iEmtAft < int(re.size()); ++iEmtAft) {
      ...
      if (!it->second->isPartial()) {
        ...
        for ( unordered_map<string,DireSplitting*>::iterator itRec = splits.begin();
  ...
  for (size_t i=0; i < splitsVec.size(); ++i) {
    ...
    bool allowed = splitNow->useFastFunctions()
                 ? splitNow->canRadiate(state,iRad,iRecNow)
                 : splitNow->canRadiate(state,iRadRec,bool_settings);
    if (!allowed) continue;

    // Get emission id.
    vector<int> re = splitNow->radAndEmt( state[iRad].id(), dip->colType);
      ...
      if (!splitNow->isPartial()) {
        ...
        for (size_t j=0; j< splitsVec.size(); ++j) {
          ...
          bool allowedRec = splitRec->useFastFunctions()
                 ? splitRec->canRadiate(state,iRecNow,iRad)
          ...
          int colTypeRec
            = state[iRecNow].isFinal() ? -dip->colType : dip->colType;
          vector<int> reRec
            = splitRec->radAndEmt( state[iRecNow].id(), colTypeRec);
...
double DireSpace::pTnext( Event& event, double pTbegAll, double pTendAll,
  ...
  if (direInfoPtr->usePTLast()) {
    ...
    pTbegAll = direInfoPtr->pTLastMin();
    direInfoPtr->usePTLast(false);
  }

  direInfoPtr->message(1)
    << "Next ISR starting from " << pow2(pTbegAll)
    << " . Current PS weights "
...
//list();
  ...
  for (int iDipEnd = 0; iDipEnd < int(dipEnd.size()); ++iDipEnd) {
    ...
    direInfoPtr->message(1) << scientific << setprecision(8)
    << "Next ISR starting scale " << pT2begDip << endl;
    ...
    if (pT2begDip > pT2sel) {
      ...
      if (pT2begDip > pT2endDip) {
        ...
        if (dipEndNow->pT2 > pT2sel) {
          ...
          //direInfoPtr->message(1) << scientific << setprecision(8)
          //<< __FILE__ //<< " " << __func__
          //<< " " << __LINE__ << " : New selected splitting "
          //<< setw(15) << splittingNowName << " at pT=" 
          //<< setw(15) << sqrt(dipEndNow->pT2) << " z = " 
          //<< setw(15) << dipEndNow->z
          //<< setw(15) << " sai=" << dipEndNow->sa1
          //<< " prob = " << kernelNow["base"] << endl; 
  ...
  //if (dipEndSel!=0) direInfoPtr->message(1) << scientific << setprecision(8)
  //  << "Next ISR splitting proposed at " << sqrt(pT2sel) << endl;
  //else direInfoPtr->message(1) << scientific << setprecision(8)
  //  << "No next ISR splitting found above " << pTendAll << endl;
...
double DireSpace::noEmissionProbability( double pTbegAll, double pTendAll,
  ...
  while ( true ) {
    ...
    // Get trial shower pT.
    double pTtrial = pTnext( dipEnds, state, startingScale, minScale, m2dip, type, s, x);
...
void DireSpace::getNewOverestimates( int idDau, DireSpaceEnd* dip,
  ...
  for ( unordered_map<string,DireSplitting*>::iterator it = splits.begin();
    ...
    if (!it->second->is_qcd) wt*=it->second->coupling()/alphaS2piOverestimate;
...
double DireSpace::getPDFOverestimates( int idDau, double tOld, double xDau,
  ...
  double pT2minPDF = pT2cut(idDau);
  if (particleDataPtr->colType(idDau) != 0) pT2minPDF = pT2min;
  double xMaxNow   = 1.-1e-6; 
  PDFscale2        = max(PDFscale2, pT2minPDF);

  bool inD         = (hasPDF(idDau)) ? beam.insideBounds(xDau, PDFscale2) : true; 
  ...
  // PDF factors for q --> qbar splitting.
    ...
    for (int i=1; i <= NTSTEPS; ++i) {
      double tNew = pT2minPDF
        + double(i)/double(NTSTEPS)*(max(tOld, pT2minPDF) - pT2minPDF);
      for (int j=1; j <= NXSTEPS; ++j) {
        double xNew = xDau + double(j)/double(NXSTEPS)*(xMaxNow-xDau);
  ...
  } else if ( particleDataPtr->isLepton(idMother)
           && particleDataPtr->isLepton(idDau)) {
    double xNew = xMaxNow - 1./10*(xMaxNow-xDau);
    double xPDFmother
      = max(getXPDF(idMother,xDau, PDFscale2, iSysNow, &beam),
            getXPDF(idMother,xNew, PDFscale2, iSysNow, &beam));
    pdfRatio = xPDFmother/xPDFdaughter;
...
void DireSpace::getNewSplitting( const Event& state, DireSpaceEnd* dip,
  ...
  string pstype = (state[dip->iRecoiler].isFinal() ? "IF" : "II" );
  ...
  // Adjust overestimate for non-QCD splittings.
  if (!splitNow->is_qcd) over *= splitNow->coupling()/alphaS2piOverestimate; 
  ...
  if ( splitNow->nEmissions() == 2 ) {
    ...
    getNextZCollQCD( dip, dip->z, 1. ); 
    // Choose sai flat in [0.0, m2Dip]
    physical = getNextVirtQCD( dip, 0.0, dip->m2Dip); 
  ...
  splitNow->splitInfo.storeInfo(name, type, dip->system, dip->system, dip->side,
    ...
    m2dipCorr, dip->pT2, dip->pT2Old, dip->z, dip->phi, m2Bef, m2s, m2r,
  ...
  if (kinRels->ISRevolutionMode==1 && state[dip->iRecoiler].isFinal()) {
    zcheck = kinRels->z_if_dire_from_cs(z, t, m2dipCorr);
    tcheck = kinRels->t_if_dire_from_cs(z, t, m2dipCorr);
  }

  double m2dipCorrTilde = dip->m2Dip;
  //if ( !physical || !inAllowedPhasespace( kinType, zcheck, tcheck, m2dipCorr,
  if ( !physical || !inAllowedPhasespace( kinType, zcheck, tcheck,
    m2dipCorrTilde, xDau, type, m2Bef, m2r, m2s, m2e, aux ) )
  ...
  //  << " " << __LINE__ << " : New"
  //  << ((state[dip->iRecoiler].isFinal() ? " IF " : " II ")) << "splitting "
  //  << setw(15) << name << " at pT=" 
  ...
  double scale2 = splitNow->couplingScale2(dip->z, dip->pT2, m2dipCorr,
    make_pair (state[dip->iRadiator].id(), state[dip->iRadiator].isFinal()),
    make_pair (state[dip->iRecoiler].id(), state[dip->iRecoiler].isFinal()));
  ...
  if (coupl > 0.) {
    ...
    if (name.find("qcd") == string::npos) {
      for ( unordered_map<string,double>::iterator it = full.begin();
        ...
        double cpl = splitNow->coupling(dip->z, dip->pT2, m2dipCorr, 
        ...
        it->second *= cpl / alphasNow(talpha, renormMultFacNow, dip->system);
      }
    }
  }
...
pair<bool, pair<double, double> > DireSpace::getMEC ( const Event& state, 
  ...
  bool hasME = weights->hasME(
    evtUtils->makeHardEvent(max(0,splitInfo->system),
    state, false));

  if (hasME) {

    direInfoPtr->message(1) << __FILE__ << " " << __func__ << " calculate mec for branching " 
    << splittingSelName
    ...
    Event newProcess( evtUtils->eventForMerging( 
      evtUtils->makeHardEvent(
      max(0,splitInfo->system), state, false), false) );
  ...
  if (abs(MECden) < 1e-15) direInfoPtr->message(1) << __FILE__ //<< " " << __func__
    << " " << __LINE__ << " : Small MEC denominator="
    << MECden << " for numerator=" << MECnum << endl;
  if (abs(MECnum/MECden) > 1e2) {direInfoPtr->message(1) << __FILE__ //<< " " << __func__
...
bool DireSpace::applyMEC ( const Event& state, DireSplitInfo* splitInfo, 
  ...
  string pstype = (state[iRecNow].isFinal() ? "IF" : "II" );
#pragma omp critical
{
  cout << scientific << setprecision(6) << setw(20) << splittingSelName
       << " " << pstype << " pT2=:" << splitInfo->kinematics()->pT2
       << ": z=:" << splitInfo->kinematics()->z << ": phi=:"
       << splitInfo->kinematics()->phi << ": mec=:" << MECnum/MECden << ":" << endl;
}
  ...
  if (hasME) {
    ...
    << endl;
        ...
        << " : Current PS weight" << weights->getShowerWeight() << endl;
    }
    if (abs(waNow) > 1.1) {
    direInfoPtr->message(1) << __FILE__ //<< " " << __func__
...
bool DireSpace::pT2nextQCD_II( double pT2begDip, double pT2sel,
  ...
  int iSysRad          = partonSystemsPtr->getSystemOf(iRadi,true);
  int iSysRec          = partonSystemsPtr->getSystemOf(iReco,true);
  bool doVariationsNow = doVariations && iSysRad<=nSysVar && iSysRec<=nSysVar;

  double pT2minPDF = pT2cutMin(&dip);
  if (particleDataPtr->colType(idDaughter) != 0) pT2minPDF = pT2min;
  ...
  do {
    ...
    //tnow  = tNextQCD( &dip, kernelPDF, tnow, pT2cutMin(&dip));
    tnow = getNextTQCD( &dip, kernelPDF, tnow, pT2endDip, pT2freeze,
    ...
    if (canUseSplitInfo) {
      ...
      if (abs(xDaughter-xCS)/xDaughter < 1e-5) {
        needNewPDF    = true;
        fullWeightsNow.clear();
        wt = fullWeightNow = overWeightNow = auxWeightNow = 0.;
        nContinue++; continue;
    ...
    double xPDFdaughterLow = getXPDF( idDaughter, xDaughter, 
      pdfScale2Old*pdfScale2Old/max(teval,pT2minPDF), iSysNow, &beam);
    ...
    // More last resort.
    if (hasPDFdau && idDaughter == 21 && pdfScale2 == pT2minPDF
      && pdfRatio>50.)
      pdfRatio = 0.;
    if (hasPDFdau && abs(idDaughter) == 11 && pdfRatio > 100.)
      pdfRatio = 0.;
...
bool DireSpace::pT2nextQCD_IF( double pT2begDip, double pT2sel,
  ...
  //                                                    : *beamAPtr );
  //BeamParticle& beam = hasPDF(event[iRadi].id())
  //                   ? (sideA ? *beamAPtr : *beamBPtr) : NULL;
  BeamParticle& beam = hasPDF(event[iRadi].id())
                     ? (sideA ? *beamAPtr : *beamBPtr) : *beamAPtr;
  ...
  do {
    ...
    if (canUseSplitInfo) {
      ...
      if ( splits[splittingNowName]->nEmissions() == 2 ) {
        ...
        // Additional factor from massive propagator.
        jacobian *= 1. / (1. - (m2ai + m2j - m2aij) / (tnow/dip.xa)) ;
      }
    ...
    if (kinRels->ISRevolutionMode==1) {
      double uCS = kinRels->ucs_if(znow,tnow,dip.m2Dip);
      if (abs(1.-2.*uCS) < 1e-5) jacobian = 0.;
      else jacobian = (1.-uCS)/(1.-2.*uCS);
    }
    ...
    pdfRatio = (hasPDFdau) ? ((inD && inM) ? xPDFmotherNew/xPDFdaughterNew : 0.)
                        : 1.;

    // More last resort.
    if (hasPDFdau && idDaughter == 21 && pdfScale2 < 1.01
      && pdfRatio > 50.)
      pdfRatio = 0.;
    if (hasPDFdau && abs(idDaughter) == 11 && pdfRatio > 100.) {
      pdfRatio = 0.;
    ...
    //<< " new pdf ratio at " << sqrt(pdfScale2) << " : " << pdfRatio
    //<< " " << xPDFdaughterNew << " (" << xDaughter << " "
    //<< 1-xDaughter << ") " << " " << xPDFmotherNew << " (" << xMother
    //<< " " << 1-xMother << ")" << endl;
    ...
    if ( fullWeightNow/auxWeightNow > 1.0) {
      ...
      //cout << __FILE__ //<< " " << __func__
        ...
        << fullWeightNow/auxWeightNow
        << " for splitting " << splittingNowName << " at pT=" << sqrt(tnow)
        << " and z=" << znow << "\t(PDF ratio=" << pdfRatio << ")" << " " << xMother << endl;
        direInfoPtr->message(1)
        << scientific << setprecision(8) << __FILE__ //<< " " << __func__
        << " : Current PS weight" << weights->getShowerWeight() << endl;
      double rescale = fullWeightNow/auxWeightNow * 1.15;
...
double DireSpace::getNextTQCD( DireSpaceEnd*, double overestimateInt,
...
bool DireSpace::getNextZCollQCD( DireSpaceEnd* dip, double zMin, double zMax,
  double, double ) {
...
bool DireSpace::getNextVirtQCD( DireSpaceEnd* dip, double, double,
  double, double) {
...
string pstype = (event[dipEndSel->iRecoiler].isFinal() ? "IF" : "II" );

  double pT2 = (dipEndSel != 0) ? dipEndSel->pT2 : 0.;
  ...
  if (hasBranched) infoPtr->errorMsg(
    "Info in DireSpace: Successfully performed " + pstype + " branching "
    + splittingSelName);
...
bool DireSpace::branch_II( Event& event, bool trial,
  ...
  int beamA = (!trial) ? event[getInA(iSysSelNow)].mother1() : 1+beamOffset;
  ...
  unordered_map<string,double> psp;
  ...
  if (name==testname && testps=="II") direInfoPtr->message(1) << __LINE__
  << " " << __func__ << " " << testps << " " << name
  << " " << event[iRecoiler].id() << " z=" << z << " tEvol=" << pT2 << endl;
  ...
  double m2e  = (!trial) ? m2ex
    : ( (split->kinematics()->m2EmtAft > 0.) ? split->kinematics()->m2EmtAft
                                            : m2ex);
  // Radiator mass.
  if ( useMassiveBeams && (abs(idDaughter) == 11 || abs(idDaughter) == 13 || abs(idDaughter) > 900000))
    m2Bef = getMass(idDaughter,1);
  if ( useMassiveBeams && (abs(idMother) == 11 || abs(idMother) == 13 || abs(idMother) > 900000))
  ...
  // Emission mass
  if ( useMassiveBeams && (abs(idSister) == 11 || abs(idSister) == 13 || abs(idSister) > 900000))
  ...
  if ( nEmissions != 2 ) {
    ...
    if (name==testname && testps=="II") direInfoPtr->message(1) << __LINE__
    << " " << __func__ << " " << testps << " " << name
    << " " << event[iRecoiler].id() << " tevol=" << pT2 << " z=" << z
    << " sai=" << saj << " " << saj - m2Emt << " sab=" << sab << " q2=" << q2
    << " sbi=" << q2 - saj - sab << " vcs=" << vCS << " xcs=" << xCS
    << " kT2=" << kT2 << " t=" << (z-vCS)*kT2 << endl;
    ...
    NewEvent.initColTag(event.lastColTag()+1000);
    ...
    << " " << event[iRecoiler].id() << " tevol=" << pT2 << " z=" << z
    << " sai=" << saj << " " << saj - m2Emt << " sab=" << sab << " q2=" << q2
    << " sbi=" << (newRecoiler.p()-sister.p()).m2Calc()
    << " " << 2.*newRecoiler.p()*sister.p() << endl;
    ...
    if (hasPDF(mother.id()) && xm > 1.) physical = false;

    // Test that enough beam momentum remains.
    double xAnew = (mother.mother1() == beamA)
              ? 2.*mother.e()      / (beamAPtr->e() + beamBPtr->e())
              : 2.*newRecoiler.e() / (beamAPtr->e() + beamBPtr->e());
    int iAold = (mother.mother1() == beamA) ? iDaughter : iRecoiler;
    int iAnew = (mother.mother1() == beamA) ? iMother : iNewRecoiler;
    double xBnew = (mother.mother1() == beamA)
              ? 2.*newRecoiler.e() / (beamAPtr->e() + beamBPtr->e())
              : 2.*mother.e()      / (beamAPtr->e() + beamBPtr->e());
    int iBold = (mother.mother1() == beamA) ? iRecoiler : iDaughter;
    int iBnew = (mother.mother1() == beamA) ? iNewRecoiler : iMother;
    ...
    if (isHardSystem && physical && doMEcorrections
      ...
      //&& pT2 > pT2minMECs && checkSIJ(event,0.)) {
      ...
      if ( evtUtils->checkSIJhard(event,iSysSelNow,false,Q2minMECs)
        && ( nFinalMaxMECs < 0
          || nFinalMaxMECs > partonSystemsPtr->sizeOut(iSysSelNow)))
        doMECreject = applyMEC (event, split, iMother, iSister, iNewRecoiler);
  ...
  // Perform 1+1 -> 3 + 1 splitting.
    ...
    double xAnew = (mother.mother1() == beamA)
              ...
              : 2.*newRecoiler.e() / (beamAPtr->e() + beamBPtr->e());
    int iAold = (mother.mother1() == beamA) ? iDaughter : iRecoiler;
    int iAnew = (mother.mother1() == beamA) ? iMother1 : iNewRecoiler;
    double xBnew = (mother.mother1() == beamA)
              ? 2.*newRecoiler.e() / (beamAPtr->e() + beamBPtr->e())
              : 2.*mother1.e()     / (beamAPtr->e() + beamBPtr->e());
    int iBold = (mother.mother1() == beamA) ? iRecoiler : iDaughter;
    int iBnew = (mother.mother1() == beamA) ? iNewRecoiler : iMother1;
    ...
    // Reject if momentum fraction is unphysical.
    if (hasPDF(mother1.id()) && xm > 1.) physical = false;
  ...
  if (name==testname && testps=="II" && pT2 > pT2minMECs) {
  ...
  if (!trial) {
    ...
    weights->setSelectedKernel(pT2, kernelSel);
    ...
    // Update resonance evolution windows in final-state shower.
    if (splits[name]->fsr)
      splits[name]->fsr->updateEvolutionWindows (idSister, pT2);
  ...
  if (doTrialNow) {
    evtUtils->shiftWZbosonID(event, iMother);
    evtUtils->shiftWZbosonID(event, iMother1);
    evtUtils->shiftWZbosonID(event, iSister);
    evtUtils->shiftWZbosonID(event, iSister1);
    evtUtils->shiftWZbosonID(event, iNewRecoiler);
  }
...
bool DireSpace::branch_IF( Event& event, bool trial,
  ...
  if (kinRels->ISRevolutionMode==1) {
    z   = kinRels->z_if_dire_from_cs(z, pT2, m2dip);
    pT2 = kinRels->t_if_dire_from_cs(z, pT2, m2dip);
  }
  ...
  if ( nEmissions != 2) { 
    ...
    int iAold = (mother.mother1() == beamA) ? iDaughter : iOther;
    int iAnew = (mother.mother1() == beamA) ? iMother : iOther;
    double xBnew = (mother.mother1() == beamA)
              ? 2.*event[iOther].e() / (beamAPtr->e() + beamBPtr->e())
              : 2.*mother.e()        / (beamAPtr->e() + beamBPtr->e());
    int iBold = (mother.mother1() == beamA) ? iOther : iDaughter;
    int iBnew = (mother.mother1() == beamA) ? iNewRecoiler : iOther;
  ...
  // Perform 1+1 -> 3 + 1 splitting.
    ...
    int idSister1(idMother);
    if (split->useForBranching) {
      idSister1 = split->emtAft2()->id;
    }
    ...
    if (!useGlobalMapIF) {
      ...
      double xAnew = (mother.mother1() == beamA)
                ...
                : 2.*event[iOther].e() / (beamAPtr->e() + beamBPtr->e());
      int iAold = (mother.mother1() == beamA) ? iDaughter : iOther;
      int iAnew = (mother.mother1() == beamA) ? iMother1 : iOther;
      double xBnew = (mother.mother1() == beamA)
                ? 2.*event[iOther].e() / (beamAPtr->e() + beamBPtr->e())
                : 2.*mother1.e()     / (beamAPtr->e() + beamBPtr->e());
      int iBold = (mother.mother1() == beamA) ? iOther : iDaughter;
      int iBnew = (mother.mother1() == beamA) ? iOther : iMother1;
      if ( hasPDF(event[iAnew].id()) && beamAPtr->size() > 0) {
        double xOld = (*beamAPtr)[iSysSelNow].x();
        (*beamAPtr)[iSysSelNow].iPos(iAnew);
        (*beamAPtr)[iSysSelNow].x(xAnew);
      ...
      if ( isHardSystem && physical && doMEcorrections && !trial
        ...
        // Store incoming particle positions and center-of-mass energy
        double shatold = (event[iAold].p() + event[iBold].p()).m2Calc();
        ...
        // Update and add newly produced particles.
        partonSystemsPtr->addOut(iSysSelNow, iSister);
        partonSystemsPtr->addOut(iSysSelNow, iSister1);
        partonSystemsPtr->replace(iSysSelNow, iRecoiler, iNewRecoiler);
        // Get new center-of-mass energy
        int iA      = getInA(iSysSelNow);
        int iB      = getInB(iSysSelNow);
        ...
        partonSystemsPtr->setInA(iSysSelNow, iAold);
        partonSystemsPtr->setInB(iSysSelNow, iBold);
        partonSystemsPtr->setSHat(iSysSelNow, shatold);
        partonSystemsPtr->replace(iSysSelNow, iNewRecoiler, iRecoiler);
        partonSystemsPtr->popBackOut(iSysSelNow);
        partonSystemsPtr->popBackOut(iSysSelNow);
      ...
      // valid after this.
      if (physical && !trial && !doMECreject) updateAfterIF( iSysSelNow, side, iDipSel,
        eventSizeOld, systemSizeOld, event, iDaughter, iRecoiler, iMother,
        iSister, iNewRecoiler, iNewOther, pT2, xm);

      // Update dipoles and beams, for second step.
      if (physical && !trial &&  !doMECreject) updateAfterIF( iSysSelNow, side, iDipSel, 0, 0,
  ...
  if (name==testname && testps=="IF" && pT2 > pT2minMECs) {
    direInfoPtr->printMessages(1); abort();
  }
  ...
  if (doTrialNow) {
    ...
    evtUtils->shiftWZbosonID(event, iSister1);
    evtUtils->shiftWZbosonID(event, iNewRecoiler);
    evtUtils->shiftWZbosonID(event, iNewRecoiler1);
  }
...
pair <Event, pair<int,int> > DireSpace::clustered_internal( const Event& state,
  int iRad, int iEmt1, int iEmt2, int iRecAft, string name ) {

  // Flags for type of radiation
  int radType = state[iRad].isFinal() ? 1 : -1;
  int recType = state[iRecAft].isFinal() ? 1 : -1;

  vector<int> iExc(createvector<int>(iRad)(iEmt1)(iRecAft));
  ...
  if ( state[iRecAft].isFinal())
    isClustered = cluster_IF(state,iRad,iEmt1,iEmt2,iRecAft,radID,name,
      RadBefore,RecBefore,NewEvent);
  else
    isClustered = cluster_II(state,iRad,iEmt1,iEmt2,iRecAft,radID,name,
      RadBefore,RecBefore,NewEvent);
...
// DEBUG
  ...
  if (!validEvent( outState, true )) {
    //direInfoPtr->eatCout();
    //cout << "not valid " << name << endl;
    //state.list(); outState.list();
...
bool DireSpace::cluster_II( const Event& state,
  int iRad, int iEmt1, int, int iRecAft, int idRadBef, string name,
  Particle& radBef, Particle& recBef, Event& partial ) {

  if (false) cout << idRadBef;

  // Calculate CS variables.
  double tDire = kinRels->tEvolDire_II(state[iRad].p(), state[iEmt1].p(), state[iRecAft].p());
  double Q2    = 2.*state[iRad].p()*state[iRecAft].p()
               - 2.*state[iRad].p()*state[iEmt1].p()
               - 2.*state[iEmt1].p()*state[iRecAft].p();
  double z     = kinRels->z_II(state[iRad].p(), state[iEmt1].p(), state[iRecAft].p());
  double tEvol = (kinRels->ISRevolutionMode==1 && state[iRecAft].isFinal()) 
    ? kinRels->tEvolCSSpace(state[iRad], state[iEmt1], state[iRecAft]) : tDire;
  ...
  double Q2tilde = Q2 + m2Bef - m2r - m2e;
  //direInfoPtr->message(1) << scientific << setprecision(15)
  //<< __LINE__ << " " << getSplittingName(state,iRad,iEmt1).front()
  //<< " : " << z << " " << pT2 << " " << Q2tilde << " " << xMin << " " << -2
  //<< " " << m2Bef << " " << m2r << " " << m2s << " " << m2e
  //<< "\t\t" << (state[iRad].p()+state[iRecAft].p()-state[iEmt1].p()).m2Calc()
  //<< endl;

  if (name==testname && testps=="II") direInfoPtr->message(1) << __LINE__
  << " " << __func__ << " " << testps << " " << name
  << " " << state[iRec].id() << " tevol=" << tEvol << " z=" << z << endl;

  if (checkLimits && !inAllowedPhasespace( 1, z, tDire, Q2tilde, xMin, -2, 
    m2Bef, m2r, m2s, m2e) ) {
    direInfoPtr->message(1) << " Clustering failed at line "
    << __LINE__ << endl;
    return false;
  }

  // Set up kinematics.
  Vec4 q(state[iRad].p() - state[iEmt1].p() + state[iRecAft].p());
  double q2 = q.m2Calc();
  ...
  Vec4 k(state[iRad].p() + state[iRecAft].p()  - state[iEmt1].p());
  Vec4 kSum(kTilde + k);
...
bool DireSpace::cluster_IF( const Event& state, 
  int iRad, int iEmt1, int iEmt2, int iRecAft, int idRadBef, string name,
  Particle& radBef, Particle& recBef, Event& partial ) {
  ...
  double tDire = (nEmissions==1)
    ? kinRels->tEvolDire_IF(state[iRad].p(), state[iEmt1].p(), state[iRecAft].p())
    : kinRels->tEvolDire_IF(state[iRad].p(), state[iEmt1].p(), state[iEmt2].p(), state[iRecAft].p());
  double z     = (nEmissions==1)
    ? kinRels->z_IF(state[iRad].p(), state[iEmt1].p(), state[iRecAft].p())
    : kinRels->z_IF(state[iRad].p(), state[iEmt1].p(), state[iEmt2].p(), state[iRecAft].p());
  double tEvol = (kinRels->ISRevolutionMode==1 && state[iRecAft].isFinal()) 
    ? kinRels->tEvolCSSpace(state[iRad], state[iEmt1], state[iRecAft]) : tDire;
  ...
  double Q2    = (nEmissions==1)
    ? kinRels->m2dip_IF(state[iRad].p(), state[iEmt1].p(), state[iRecAft].p())
    : kinRels->m2dip_IF(state[iRad].p(), state[iEmt1].p(), state[iEmt2].p(),
        state[iRecAft].p());
  // Get dipole 4-momentum.
  ...
  if ( nEmissions == 2 ) {
    ...
    double xa = kinRels->xa_IF(state[iRad].p(), state[iEmt1].p(), state[iEmt2].p(), state[iRecAft].p());
    double m2aij = m2Bef;

    double m2a(state[iRad].p().m2Calc()), m2i(state[iEmt1].p().m2Calc()),
      m2j(state[iEmt2].p().m2Calc()), m2k(state[iRecAft].p().m2Calc());

    double m2ai  = sai + m2a + m2i;
    xCS          = (q2 - m2ai - m2a - m2i)
                 / (q2 - m2ai - m2a - m2i - tDire * xa/z);
    double xCDST = xCS * ( 1. - (m2aij-m2ai-m2j)/ (q2-m2ai-m2j-m2k) );
    xOld = xNew*xCDST;
    aux.push_back(Q2);
    aux.push_back(q2);
    aux.push_back(tDire);
  ...
  if (name==testname && testps=="IF") direInfoPtr->message(1) << __LINE__
  << " " << __func__ << " " << testps << " " << name
  << " " << state[iRec].id() << " tevol=" << tEvol << " z=" << z << endl;

  double Q2tilde = Q2 + m2Bef - m2r - m2e;
  if (checkLimits && !inAllowedPhasespace( 1, z, tDire, Q2tilde, xMin, 2, m2Bef,
    m2r, m2s, m2e, aux) )
    return false;
  ...
  if (!useGlobalMapIF) {
    ...
    Vec4 paj(pRad-pEmt);
    Vec4 pk(pRec);
    Vec4 pTk(pk.px()+pEmt.px(),pk.py()+pEmt.py(),0.,0.);
    q.p(paj-pk);
    Vec4 qpar(q+pTk);
    ...
    double sjk   = (pRec + pEmt).m2Calc();

    // Now construct radiator in lab frame.
    pRadBef = ( pRad - q*pRad/q2par * qpar )
...
int DireSpace::FindParticle( const Particle& particle, const Event& event,
  ...
  if ( checkStatus && index >-1 && event[index].status() != particle.status() )
    index = -1;
...
map<string, double> DireSpace::getStateVariables (const Event& state,
  ...
  if (rad > 0 && emt > 0 && rec > 0) {
    ...
    double tEvol = kinRels->tEvolSpace ( state[rad], state[emt], state[rec]);
    double z     = kinRels->zSpace ( state[rad], state[emt], state[rec]);
    ret.insert(make_pair("t",tEvol));
    ret.insert(make_pair("tRS",tEvol));

    // Get scales for running coupling and PDF.
    double m2dip = kinRels->m2dipSpace ( state[rad], state[emt], state[rec]);
    ret.insert(make_pair("m2dip", m2dip));
    double mu2 = tEvol;
    //if (name != "") mu2 = (*splittingsPtr)[name]->couplingScale2(z, tEvol,
...
double DireSpace::getSplittingProb( const Event& state, int iRad,
  int iEmt1, int iEmt2, int iRecAft, string name, bool checkPTMINcut) {

  direInfoPtr->message(1) << __func__ << " " << __LINE__ << " "
  << name << " checkcut=" << checkPTMINcut;
  string pstype=(state[iRecAft].isFinal() ? "IF" : "II" );
  ...
  int nEmissions = (iEmt2<0) ? 1 : 2;
  double z     = (nEmissions==1)
               ? kinRels->zSpace(state[iRad], state[iEmt1], state[iRecAft])
               : kinRels->zSpace(state[iRad], state[iEmt1], state[iEmt2], state[iRecAft]);
  double tDire = (nEmissions==1) 
               ? kinRels->tEvolDireSpace(state[iRad], state[iEmt1], state[iRecAft])
               : kinRels->tEvolDireSpace(state[iRad], state[iEmt1], state[iEmt2], state[iRecAft]);
  double m2D   = (nEmissions==1) 
               ? kinRels->m2dipSpace(state[iRad], state[iEmt1], state[iRecAft])
               : kinRels->m2dipSpace(state[iRad], state[iEmt1], state[iEmt2], state[iRecAft]);
  double tEvol = (kinRels->ISRevolutionMode==1 && state[iRecAft].isFinal()) 
    ? kinRels->tEvolCSSpace(state[iRad], state[iEmt1], state[iRecAft]) : tDire;
  ...
  double m2e   = state[iEmt1].p().m2Calc();
  double m2s   = state[iRecAft].p().m2Calc();
  if (particleDataPtr->isQuark(idRadBef)
   || particleDataPtr->isGluon(idRadBef)) m2Bef = 0.;
  int type     = (state[iRecAft].isFinal()) ? 1 : -1;

  // Disallow below cut-off.
  if (checkPTMINcut && pT2cut(state[iEmt1].id()) > tEvol) {
    if (name==testname && testps==pstype) direInfoPtr->message(1) << __LINE__
    << " " << __func__ << " " << pstype << " " << name << " "
    << state[iRecAft].id() << " " << __func__ << " " << pstype
    << " tevol=" << tEvol << " z=" << z << endl;
    return 0.;
  }
  if (checkPTMINcut && !splits[name]->aboveCutoff( tEvol, state[iRad],
        state[iRecAft], 0, partonSystemsPtr)) {
    if (name==testname && testps==pstype) direInfoPtr->message(1) << __LINE__
    << " " << __func__ << " " << pstype << " " << name
    << " " << state[iRecAft].id() << " " << __func__ << " " << pstype
    << " tevol=" << tEvol << " z=" << z << endl;
  ...
  double xMin = hasPDF(idRadBef) ? xDau : 0.;

  // Get phi angle.
  int massSign = type > 0 ? 1 : -1;
  pair<Vec4, Vec4> pTdirection = getTwoPerpendicular(state[iRad].p(),
    massSign*state[iRecAft].p()+state[iEmt1].p());
  double px= -pTdirection.first*state[iEmt1].p();
  double py= -pTdirection.second*state[iEmt1].p();
  ...
  if (nEmissions==2) {
    ...
    pTdirection = getTwoPerpendicular(state[iRecAft].p(),
  ...
  vector <double> aux;
  if ( nEmissions == 2 ) {
    double sai = abs(2.*state[iRad].p()*state[iEmt1].p());
    double xa  = kinRels->xaSpace(state[iRad], state[iEmt1], state[iEmt2], state[iRecAft]);
    double m2aij = m2Bef;
    double m2a(state[iRad].p().m2Calc()), m2i(state[iEmt1].p().m2Calc()),
      m2j(state[iEmt2].p().m2Calc()), m2k(state[iRecAft].p().m2Calc());
    double q2    = (state[iRad].p() - state[iEmt1].p()
                   -state[iEmt2].p()- massSign*state[iRecAft].p()).m2Calc();
    double m2ai  = sai + m2a + m2i;
    xCS          = (q2 - m2ai - m2a - m2i)
                 / (q2 - m2ai - m2a - m2i - tDire * xa/z);
    double xCDST = xCS * ( 1. - (m2aij-m2ai-m2j)/ (q2-m2ai-m2j-m2k) );
    xMin = (particleDataPtr->colType(idRadBef) != 0) ? xMot*xCDST : 0.;
    aux.push_back(m2D);
    aux.push_back(q2);
    aux.push_back(tDire);
    aux.push_back(sai);
  ...
  << " " << __func__ << " " << pstype << " " << name
  << " " << state[iRecAft].id() << " " << __func__ << " " << pstype
  << " tevol=" << tEvol << " z=" << z << " xbj=" << xMin << endl;

  double m2dipCorrTilde = m2dipCorr + m2Bef - m2r - m2e;
  if (!inAllowedPhasespace( 1, z, tDire, m2dipCorrTilde, xMin, type, m2Bef,
    m2r, m2s, m2e)) { return 0.0; }

  //direInfoPtr->message(1) << __LINE__ << " " << name << " : z=" << z
  //     << " pT2=" << tDire << " m2="
  //     << m2dipCorr << " x=" << xMin << " type=" << type << " m2rb=" << m2Bef
  //     << " m2ra=" << m2r << " m2sa=" << m2s << " m2ea=" << m2e << endl;

  // Calculate splitting probability.
  double p = 0.;

  // Setup splitting information.
  pair <Event, pair<int,int> > born(clustered_internal( state, iRad, iEmt1, iEmt2, iRecAft, name ));
  //int nEmissions = splits[name]->nEmissions();
...
vector<int> DireSpace::getRecoilers( const Event& state, int iRad, int iEmt1,

...
#define DIRE_TIMES_VERSION "2.004"
...
class EvolutionWindow {
  ...
  public:
    ...
    mutable double tMax;
    mutable double tMin;
    vector<int> evolvingIds;
    bool isResonant;
    int iWindow;

    EvolutionWindow(double min, double max, int iw, int id = 0,
      bool isRes = false) :
      tMax(max), tMin(min), isResonant(isRes), iWindow(iw) {
      evolvingIds.push_back(id); }

    EvolutionWindow(const EvolutionWindow & w) :
      tMax(w.tMax), tMin(w.tMin), evolvingIds(w.evolvingIds),
      isResonant(w.isResonant), iWindow(w.iWindow) {}
    ...
    bool operator<(const EvolutionWindow& ew) const { 
      return (this->tMax > ew.tMax); }
...
public:
  ...
  void reinitPtr(Info* infoPtrIn, Settings* settingsPtrIn,
       ...
       MergingHooks* mergingHooksPtrIn, DireSplittingLibrary* splittingsPtrIn,
  ...
  virtual Event clustered( const Event& state, int iRad, int iEmt, int iRec,
    ...
    pair <Event, pair<int,int> > reclus
      = clustered_internal(state, iRad, iEmt, -1, iRec, name);
  ...
  virtual Event clustered( const Event& state, int iRad, int iEmt1,
    int iEmt2, int iRec,
    string name) {
    pair <Event, pair<int,int> > reclus = 
      clustered_internal(state, iRad, iEmt1, iEmt2, iRec, name);
    if (reclus.first.size() > 0)
      reclus.first[0].mothers(reclus.second.first,reclus.second.second);
    return reclus.first;
  }
  ...
  bool cluster_FF( const Event& state, int iRad,
    int iEmt1, int iEmt2, int iRec, int idRadBef, string name,
    Particle& radBef, Particle& recBef);
  bool cluster_FI( const Event& state, int iRad,
    int iEmt1, int iEmt2, int iRec, int idRadBef, string name,
    Particle& radBef, Particle& recBef);

  /*double cosPhi_a_ij_b ( const Vec4& rad, const Vec4& emt1, const Vec4& emt2,
    const Vec4& rec) {
    double sab = rad*rec;
    double sia = rad*emt1;
    double sjb = rec*emt2;
    double sib = rec*emt1;
    double sja = rad*emt2;
    double sij = emt1*emt2;
    double sa  = rad.m2Calc();
    double sb  = rec.m2Calc();
    return (sab * (sia*sjb + sib*sja - sij*sab)
              - (sa*sib*sjb + sb*sia*sja - sa*sb*sij))
           / (sqrt(kT2_a_i_b(rad,emt1,rec)*kT2_a_i_b(rad,emt2,rec))
              * lABC(2.*sab+sa,sa,sb)/4.);
  }
  double kT2_a_i_b ( const Vec4& rad, const Vec4& emt, const Vec4& rec) {
    double sab = rad*rec;
    double sia = rad*emt;
    double sib = rec*emt;
    double sa  = rad.m2Calc();
    double sb  = rec.m2Calc();
    double si  = emt.m2Calc();
    return (2.*sab*sia*sib - sa*sib*sib - sb*sia*sia - si*sab*sab + si*sa*sb)
           /(lABC(2.*sab+sa,sa,sb)/4.);
  }*/
  ...
  virtual vector<string> getSplittingName( const Event& state, int iRad,
    int iEmt)
    { return (state[iRad].isFinal()
       ? splittingsPtr->getSplittingName(state,iRad,iEmt)
       : vector<string>()); }
  virtual vector<string> getSplittingName( const Event& state, int iRad,
    int iEmt1, int iEmt2)
    { return (state[iRad].isFinal()
        ? splittingsPtr->getSplittingName(state,iRad,iEmt1,iEmt2)
        : vector<string>()); }
  ...
  virtual double getSplittingProb( const Event& state, int iRad,
    int iEmt1, int iEmt2, int iRec, string name, bool checkPTMINcut = false);

  bool allowedSplitting( const Event&, int, int) { return true; }

  virtual vector<int> getRecoilers( const Event& state, int iRad, int iEmt,
    string name);
  virtual vector<int> getRecoilers( const Event& state, int iRad, int iEmt1,
  ...
  bool validMomentum( const Vec4& p, int id, int status) {
    BeamParticle* beam = 0;
    if (beamAPtr != 0 || beamBPtr != 0) {
      beam = (beamAPtr != 0 && particleDataPtr->isHadron(beamAPtr->id()))
           ? beamAPtr
           : (beamBPtr != 0 && particleDataPtr->isHadron(beamBPtr->id()))
              ? beamBPtr : 0;
    }
    bool usePDFmass = usePDFmasses && isLHAPDF;
    return evtUtils->validMomentum(p, id, status, beam, usePDFmass);
  }
  ...
  bool validEvent( const Event& state, bool isProcess = false,
    ...
    bool usePDFmass = usePDFmasses && isLHAPDF;
    return evtUtils->validEvent(state,beam,usePDFmass,isProcess,iSysCheck);
  }

  // Check that mother-daughter-relations are correctly set.
  bool validMotherDaughter( const Event& state ) {
    return evtUtils->validMotherDaughter(state);
  }
...
private:
  ...
  static const double LEPTONZMAX, LEPTONXMIN, LEPTONXMAX;
  double CA, CF, TR, NC;
...
static const string testps;
  ...
  static const double MCMIN, MBMIN, SIMPLIFYROOT, XMARGIN, XMARGINCOMB,
         ...
         PT2MIN_PDF_IN_OVERESTIMATE, PT2_INCREASE_OVERESTIMATE, KERNEL_HEADROOM;
  ...
  int    pTmaxMatch, pTdampMatch, alphaSorder, alphaSnfmax, alphaEMorder,
         ...
         nFinalMaxMECs,kernelOrder, kernelOrderMPI, nMPI, asScheme, nSysVar;
  ...
  bool usePDFalphas, usePDFmasses, useSummedPDF, usePDF, isLHAPDF,
       usePDFlepton, useSystems, useMassiveBeams, suppressLargeMECs,
       allowMECsBelowCutoff;
  ...
  void setKernelOrder(int in) { kernelOrder = in; }
  int getKernelOrder() { return kernelOrder; }
  ...
  void pT2nextQCD( double pT2begDip, double pT2sel,
    DireTimesEnd& dip,
    Event& event, double pT2endForce = -1., double pT2freeze = 0.,
    bool forceBranching = false);
  bool pT2nextQCD_FF( double pT2begDip, double pT2sel,
    DireTimesEnd& dip,
    const Event& event, double pT2endForce = -1., double pT2freeze = 0.,
    bool forceBranching = false);
  bool pT2nextQCD_FI( double pT2begDip, double pT2sel,
    DireTimesEnd& dip,
    const Event& event, double pT2endForce = -1., double pT2freeze = 0.,
    bool forceBranching = false);

  bool pT2nextQCD_FI_test( double pT2begDip, double pT2sel,
    DireTimesEnd& dip,
    const Event& event, double pT2endForce = -1., double pT2freeze = 0.,
    bool forceBranching = false);


  void tNextDecay( double pT2begDip, double pT2sel,
    DireTimesEnd& dip,
    Event& event, double pT2endForce = -1., double pT2freeze = 0.,
    bool forceBranching = false);
  bool tNextDecay_FF( double pT2begDip, double pT2sel,
    DireTimesEnd& dip,
    const Event& event, double pT2endForce = -1., double pT2freeze = 0.,
    bool forceBranching = false);
  bool tNextDecay_FI( double pT2begDip, double pT2sel,
    DireTimesEnd& dip,
    const Event& event, double pT2endForce = -1., double pT2freeze = 0.,
    bool forceBranching = false);

  double tForceResDecMax(int id) { return pow2(2.*particleDataPtr->m0(id)); }
  double tForceResDecMin(int id) { return pow2(1.*particleDataPtr->m0(id)); }

  int iWindowNow, iWindowSel;
  multiset<EvolutionWindow> evolutionWindows;

  void updateEvolutionWindows (int idEmt, double tEvol, int iNow = -1) {
//cout << __func__ << "update windows at t=" << tEvol << " " << iNow << " " << evolutionWindows.size()<< " " << idEmt << " " << particleDataPtr->isResonance(idEmt) << endl;
    if (iNow>-1) {
      for (auto w = evolutionWindows.begin(); w != evolutionWindows.end(); ++w)
        if ((*w).iWindow == iNow) (*w).tMax = tEvol;
    }
    if (particleDataPtr->isResonance(idEmt)) {
      double tMin = tForceResDecMin(idEmt);
      double tMax = tForceResDecMax(idEmt);
      evolutionWindows.insert(
        EvolutionWindow(tMin, tMax, evolutionWindows.size(), idEmt, true));
    }
    return;
  }

  bool isAllowedSplit(DireTimesEnd* dip, double tCutNow) {
    if (dip->pT2 < tCutNow) return false;
    return true;
  }

  void updateWeightsAfterRejection(const double & tnow, const double & teval,
    const unordered_map<string,double> & fullWeightsNow,
    const double & fullWeightNow, const double & overWeightNow,
    const double& auxWeightNow);

  void updateWeightsAfterAcceptance(const double & tnow, const double & teval,
    const unordered_map<string,double> & fullWeightsNow,
    const double & fullWeightNow, const double & overWeightNow,
    const double& auxWeightNow);

  void setupForNextEvolutionStep( double &wt, DireTimesEnd& dip,
    unordered_map<string,double> & fullWeightsNow, double & fullWeightNow,
    double & overWeightNow, double& auxWeightNow);

  void getMassesforSplitting(const DireTimesEnd & dip,
  ...
  void setVetoWeight(const double & tnow, const double & z,
    double & fulWeightNow, double & auxWeightNow, double & wt);

  void calculateMURvariations(const bool& doVariationsNow, const double & tnow,
    const double& talpha, const int & system, const bool & forceFixedAs,
    unordered_map<string,double>& fullWeightsNow);

  void calculatePDFvariations(const bool& doVariationsNow, const double & tnow,
    const bool & hasPDFrec,
  ...
  pair<double,double> getNextEvolutionScale( DireTimesEnd* dip,
    const Event & event,
    double overestimateInt, double tOld, double tMin, double pT2min,
    double tFreeze, int algoType, bool fixedCoupling,
    int typeEvolutionMeasure);

  double getIntBW(int idRad, double tMax, double tMin);

  int getEvolutionMeasureType(DireTimesEnd* dip, const Event & event,
    double zIntOverestimate [],
    double tMax, double tMin);

  double getNextTQCD( DireTimesEnd*, double overestimateInt,
    double tOld, double tMin, double tFreeze=0., int algoType = 0,
    bool fixedCoupling = false);
  bool getNextZCollQCD( DireTimesEnd* dip, double zMin, double zMax,
    double tMin = 0., double tMax = 0.);
  bool getNextVirtQCD( DireTimesEnd* dip, double tMin, double tMax,
    double zMin =-1., double zMax =-1.);

  double getNextQ2Res( DireTimesEnd*,
    double widthRes, double mRes, double smax,
    double smin);
  ...
  void alphasReweight(double t, double talpha, int iSys, bool forceFixedAs,
    ...
    double renormMultFacNow, double numeratorMultFac = 1.);
  ...
  void getNewSplitting( const Event&, DireTimesEnd*, double, double, double,
    double, double, int, string, bool, int, int&, int&, double&, double&, 
    unordered_map<string,double>&, double&);

  pair<bool, pair<double,double> > getMEC ( const Event& state, 
    DireSplitInfo* splitInfo);
  bool applyMEC ( const Event& state, DireSplitInfo* splitInfo, int = 0,
    int = 0, int = 0);
  ...
  double getMass(int id, int strategy, double mass = 0.) {
    ...
    bool usePDFmass = usePDFmasses && isLHAPDF;
    return evtUtils->getMass(id,strategy,beam,usePDFmass,mass);
  }
  ...
  bool inAllowedPhasespace(int kinType, double z, double pT2, double m2dip,
    ...
    vector<double> aux = vector<double>(), DireSplitting* split = 0);
...
public:
  ...
  vector<DireSplitting*> splitsVec;

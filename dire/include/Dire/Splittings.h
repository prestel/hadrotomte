...
#define DIRE_SPLITTINGS_VERSION "2.004"
...
public:  
  ...
  DireSplitting() :
      ...
      is_isr(false), is_dire(false), doVariations(false), doMECs(false),
      nameHash(0), enhanceSave(1.0), isEnhancedSave(false), 
      forceSplitting(false) {
    radAndEmtArray[0] = radAndEmtArray[1] = radAndEmtArray[2] = 0; }
  DireSplitting(string idIn, int softRS, Settings* settings,
    ...
    DireInfo* direInfo, SLHAinterface* slhaInterface = NULL) :
      ...
      direInfoPtr(direInfo), slhaInterfacePtr(slhaInterface),
      is_qcd(false), is_qed(false), is_ewk(false), is_fsr(false),
      is_isr(false), is_dire(false), doVariations(false), doMECs(false),
      nameHash(0), enhanceSave(1.0), isEnhancedSave(false),
      forceSplitting(false) {
...
public:
  ...
  bool is_qcd, is_qed, is_ewk, is_fsr, is_isr, is_dire, doVariations,
       doMECs;
  ulong nameHash;
  ...
  double enhanceSave, enhance;
  bool isEnhancedSave;
  void setIsEnhanced (bool in) { isEnhancedSave = in; }
  bool isEnhanced ()           { return isEnhancedSave; }
  double getEnhanceFactor ()   { return enhanceSave; }
  ...
  virtual double tCutoff( const Particle&, const Particle&, 
    PartonSystems* = NULL) { return -1.; }
  ...
  // the splitting.
  virtual int * radAndEmtArr (int, int)
  ...
  virtual vector <int> recPositions( const Event&, int, int)
    {return vector<int>();} 
  virtual vector <int> recPositions( const Event&, int, int, int)
    {return vector<int>();} 
  ...
  virtual double coupling (double = -1., double = -1., double = -1.,
  ...
  virtual void clearKernelVals() {
    kernelVals.clear();
  }

  virtual void adjustKernelVals(unordered_map<string,double>& in,
    const Event&, double);
  virtual void storeKernelVals(unordered_map<string,double> in);
  virtual void storeKernelVal(string key, double val);
  virtual bool zero();
  ...
  void setTimesDecPtr(DireTimes* fsrIn) { fsrDec=fsrIn;}
  ...
  virtual double getZDire(bool,bool, double z, double, double) { return z;}
  virtual double getTDire(bool,bool, double, double t, double) { return t;}
  ...
  virtual int typeEvolutionMeasure() { return 0; }

  bool forceSplitting;
  void setForceSplitting() { forceSplitting = true; }
  void unsetForceSplitting() { forceSplitting = false; }
  bool giveUpAndForceSplitting() { return forceSplitting; }

  int radAndEmtArray [3];

// DireSplitInfo.h is a part of the DIRE plugin to the PYTHIA event generator.
...
#define DIRE_SPLITINFO_VERSION "2.004"
...
public:
  ...
  void store2to4kine( double m2DipIn, double pT2In, double zIn, double phiIn,
    double saiIn, double xaIn, double phi2In, double xAftIn = -1.) { m2Dip = m2DipIn;
  ...
  unordered_map<string,double> getKinInfo() {
    return create_unordered_map<string,double> ("m2Dip",m2Dip)("pT2",pT2)("pT2Old",pT2Old)
...
public:
  ...
  DireSplitInfo ( const Event& state, int iRadBefIn, int iRecBefIn, 
    ...
    DireSingleColChain iSiblingsIn = DireSingleColChain() ) :
      ...
      terminateEvolution(false), checkCuts(true),
      idRadInt(0), idEmtInt(0), colRadInt(-1), acolRadInt(-1), colEmtInt(-1),
      acolEmtInt(-1), swapped(false), unitKernel(false), iSiblings(iSiblingsIn)
      { init(state); }
  ...
  DireSplitInfo ( const Event& state, int iRadBefIn, int iRecBefIn,
    ...
    useForBranching = terminateEvolution = false; checkCuts = true;
    idRadInt = idEmtInt = 0;
    colRadInt = acolRadInt = colEmtInt = acolEmtInt = -1;
    swapped = unitKernel = false;
    init(state); }

  DireSplitInfo ( const Event& state, int iRadAftIn, int iRecAftIn, int iEmtAftIn,
  ...
  DireSplitInfo ( const DireSplitInfo& s) {
    ...
    checkCuts = s.checkCuts;
    idRadInt   = s.idRadInt;
    idEmtInt   = s.idEmtInt;
    colRadInt  = s.colRadInt;
    acolRadInt = s.acolRadInt;
    colEmtInt  = s.colEmtInt;
    acolEmtInt = s.acolEmtInt;
    swapped    = s.swapped;
    unitKernel = s.unitKernel;
  ...
  bool useForBranching, terminateEvolution, checkCuts;
  ...
  bool useForBranchingStore, terminateEvolutionStore, checkCutsStore;

  int idRadInt, idEmtInt, colRadInt, acolRadInt, colEmtInt, acolEmtInt;
  bool swapped, unitKernel;

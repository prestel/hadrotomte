...
#define DIRE_SPACE_VERSION "2.004"
...
public:
  ...
  DireSpace(Pythia8::Pythia* pythiaPtr) :
    ...
    strengthIntAsym(0.), pT2minVariations(0.), pT2minEnhance(0.),pT2minMECs(0.),
    Q2minMECs(0.),
    alphaS2piOverestimate(0.), usePDFalphas(false), usePDFmasses(false),
    useSummedPDF(false), usePDF(true), isLHAPDF(false), usePDFlepton(false),
    useSystems(true), useGlobalMapIF(false), forceMassiveMap(false),
    useMassiveBeams(false), suppressLargeMECs(false),
    allowMECsBelowCutoff(false) {
    beamOffset        = 0;
    pTdampFudge       = 0.;
  ...
  void setWeightContainerPtr(DireWeightContainer* weightsIn) { weights = weightsIn;}
  ...
  virtual Event clustered( const Event& state, int iRad, int iEmt1,
    int iEmt2, int iRecAft,
    string name) {
    pair <Event, pair<int,int> > reclus
      = clustered_internal(state, iRad, iEmt1, iEmt2, iRecAft, name);
  ...
  bool cluster_II( const Event& state, int iRad,
    int iEmt1, int iEmt2, int iRecAft, int idRadBef, string name,
    Particle& radBef, Particle& recBef, Event& partialState);
  ...
  virtual vector<string> getSplittingName( const Event& state, int iRad, int iEmt)
    { return (!state[iRad].isFinal()
  ...
  virtual double getSplittingProb( const Event& state, int iRad,
    int iEmt1, int iEmt2, int iRecAft, string, bool checkPTMINcut = false);
...
static const string testps;
  ...
  int    pTmaxMatch, pTdampMatch, alphaSorder, alphaSnfmax,
         ...
         kernelOrderMPI, nWeightsSave, nMPI, asScheme, nSysVar;
  ...
  bool  usePDFalphas, usePDFmasses, useSummedPDF,  usePDF, isLHAPDF,
        usePDFlepton, useSystems, useGlobalMapIF, forceMassiveMap,
        useMassiveBeams, suppressLargeMECs, allowMECsBelowCutoff;
  ...
  double getNextTQCD( DireSpaceEnd*, double overestimateInt,
    double tOld, double tMin, double tFreeze=0., int algoType = 0,
    bool fixedCoupling = false);
  bool getNextZCollQCD( DireSpaceEnd* dip, double zMin, double zMax,
    double tMin = 0., double tMax = 0.);
  bool getNextVirtQCD( DireSpaceEnd* dip, double tMin, double tMax,
  ...
  double evalpdfstep(int idRad, double pT2, double m2cp = -1.,
    ...
    // More steps for lepton.
    if ( particleDataPtr->isLepton(idRad) && usePDFlepton && pT2 < 0.5)
      ret = 1.0;

...
void Sigma2ff2fftgmZ::sigmaKin() {
  ...
  //double idA = beamAPtr->id();
  //double idB = beamBPtr->id();
...
// << " " << uH
// << " " << sH + tH + uH
// << " " << -tH/x1Save*(1.-x1Save) + pow2(mA) 
// << " " << -tH/x2Save*(1.-x2Save) + pow2(mB) 
// << endl;

//cout << " sH2 " << sH2 << " uH2 " << uH2 << " tH2 " << tH2 << " alpEM " << alpEM << endl;
//cout << sigma0 << " " << 2.*(sH2 + uH2) / tH2 << " " << sigmagmgm << endl;
//abort();
count++;
bla.fill(pow(abs(tH2),0.5));
blawt.fill(pow(abs(tH2),0.5), sigmagmgm);
...
double norm1 = 0.;
for (int i=1; i < tmpbla.getBinNumber(); ++i)
  norm1 += tmpbla.getBinContent(i);
double norm2 = 0.;
for (int i=1; i < tmpblawt.getBinNumber(); ++i)
  norm2 += tmpblawt.getBinContent(i);
...
/*  // Write histograms to file
  ...
  write.open("blawt.dat");
  tmpblawt.table(write);
  write.close();
*/
}
...
//cout << __FILE__ << " " << beamAPtr->m() << " " << beamAPtr->e() << " " << x1Save << endl;
//cout << __FILE__ << " " << beamBPtr->m() << " " << beamBPtr->e() << " " << x2Save << endl;

  if (particleDataPtr && beamAPtr && beamBPtr) {
    double xLeft = particleDataPtr->isHadron(idA)
                 ? 1. - beamAPtr->m()/beamAPtr->e() - x1Save
                 : 1. - beamBPtr->m()/beamBPtr->e() - x2Save;

//if (xLeft>0.) cout << "success!!" << endl;
//else cout << "failure " << xLeft << endl;

    double w2 = particleDataPtr->isHadron(idA)
              ? -tH/x1Save*(1.-x1Save) + pow2(mA)
              : -tH/x2Save*(1.-x2Save) + pow2(mB);
    if (w2 < 2.) sigmagmgm = sigmagmZ = sigmaZZ = sigma0 = 0.;
    if (xLeft< 0.) sigmagmgm = sigmagmZ = sigmaZZ = sigma0 = 0.;
  }
...
//cout << " sigma " << sigma << " sigmagmgm " << sigmagmgm << " sigmagmZ " << sigmagmZ << " sigmaZZ " << sigmaZZ << endl;

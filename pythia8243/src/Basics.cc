...
ostream& operator<<(ostream& os, const Vec4& v) {
  ...
  int extraw=7;
  os << scientific << setprecision(6) << " " << setw(9+extraw) << v.xx << " "
     << setw(9+extraw) << v.yy << " " << setw(9+extraw) << v.zz << " " << setw(9+extraw)
     << v.tt << " (" << setw(9+extraw) << v.mCalc() << ")\n";
  return os;
}
...
pair<Vec4,Vec4> getTwoPerpendicular(const Vec4& v1, const Vec4& v2) {
  ...
  if ( abs(nPerp.pAbs()) < TINY) {
    Vec4 aux;
    if (abs(v1.px()) > TINY)      aux.p(v1.py(),v1.px(),v1.pz(),v1.e());
    else if (abs(v1.py()) > TINY) aux.p(v1.px(),v1.pz(),v1.py(),v1.e());
    else if (abs(v1.pz()) > TINY) aux.p(v1.pz(),v1.py(),v1.px(),v1.e());

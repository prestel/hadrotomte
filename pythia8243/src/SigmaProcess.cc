...
//cout << initPS << " " << samexGamma << " " << useNewXvalues << " " << x1Save << " " << x2Save << " " << x1New << " " << x2New << " " << 1.-x1Save << " " << 1.-x2Save << " " << sizeBeamA() << sizeBeamB() << endl;

  bool checkX1 = false;
  for (int j = 0; j < sizeBeamA(); ++j)
    if ( abs(inBeamA[j].id) > 100
      || (particleDataPtr->isLepton(abs(inBeamA[j].id))
       && settingsPtr->flag("PDF:lepton")) ) checkX1 = true;
  bool checkX2 = false;
  for (int j = 0; j < sizeBeamB(); ++j)
    if ( abs(inBeamB[j].id) > 100
      || (particleDataPtr->isLepton(abs(inBeamB[j].id))
       && settingsPtr->flag("PDF:lepton")) ) checkX2 = true;

  double min=3e-6;
  bool largeX1 = (1.-x1Save<min);
  bool largeX2 = (1.-x1Save<min);

  bool vetoX1 = checkX1 && largeX1;
  bool vetoX2 = checkX2 && largeX2;

  if (vetoX1 || vetoX2) return 0.;


  //if      (initPS        && (1.-x1Save<1e-5 || 1.-x2Save<1e-5)) { cout <<"aaaaaaa" << endl; return 0.;}
  //else if (useNewXvalues && (1.-x1New<1e-5  || 1.-x2New<1e-5))  { cout << "bbbb" << endl; return 0.;}
  //else if (1.-x1Save<1e-6 || 1.-x2Save<1e-6) { cout <<"aaaaaaa" << endl; return 0.;}
/*  double min=3e-6;
  if ( (initPS        && (1.-x1Save<min || 1.-x2Save<min)) 
    || (useNewXvalues && (1.-x1New<min  || 1.-x2New<min))
    || (1.-x1Save<min || 1.-x2Save<min)) return 0.;
*/
...
//cout << __LINE__ << " pdf " << inBeamA[j].pdf << "sigma " << inPair[i].pdfSigma << endl;
...
//cout << __LINE__ << " sigmaSum " << sigmaSumSave << endl;
  }
...
//cout << " sH " << sH << " uH " << uH << " tH " << tH << " pT2 " << pT2 << endl;

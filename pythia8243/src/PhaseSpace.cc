...
const int    PhaseSpace::NMAXTRY        = 10; 
...
const double PhaseSpace::SAFETYMARGIN   = 2.0;
...
void PhaseSpace::init(bool isFirst, SigmaProcess* sigmaProcessPtrIn,
  ...
  scanInitialX         = settingsPtr->flag("PhaseSpace:scanInitialX");
...
bool PhaseSpace::setupSampling123(bool is2, bool is3) {
  ...
  for (int itry=0; itry< 1000000; ++itry) {
    ...
    if (itry>0) posTauNow = rndmPtr->flat();

  for (int iTau = 0; iTau < nTau; ++iTau) {
    //double posTau = 0.5;
    double posTau = posTauNow;
...
bool PhaseSpace::trialKin123(bool is2, bool is3, bool inEvent) {
  ...
  if (sigmaNw > sigmaMx) {
    ...
    //cout << "Warning in PhaseSpace2to2tauyz::trialKin: maximum for cross section violated " << scientific << setprecision(8) << sigmaNw/sigmaMx << " " << x1H << " " << x2H << endl;

if (sigmaNw/sigmaMx>1e2) {cout << "very large violation!!!" << endl; }
    ...
    // Violation strategy 2: weight event (done in ProcessContainer).
      ...
      cout << " PYTHIA Maximum for " << sigmaProcessPtr->name()
           << " exceeded by factor " << setprecision(3) << violFact
  ...
  // Before leaving, check that all x have been sampled at least a few
  // times.
  if (x2H> 0.0 && x2H<0.1) n01++;
  if (x2H> 0.1 && x2H<0.2) n02++;
  if (x2H> 0.2 && x2H<0.3) n03++;
  if (x2H> 0.3 && x2H<0.4) n04++;
  if (x2H> 0.4 && x2H<0.5) n05++;
  if (x2H> 0.5 && x2H<0.6) n06++;
  if (x2H> 0.6 && x2H<0.7) n07++;
  if (x2H> 0.7 && x2H<0.8) n08++;

  //int ncount=10000;
  int ncount=1000;
  int nnow = n01+n02+n03+n04+n05+n06+n07+n08;
  if (scanInitialX && !inEvent && !hasTwoPointParticles)
    return ((nnow > 1000000) || ( n01>-1
    && n02>ncount
    && n03>ncount
    && n04>ncount
    && n05>ncount
...
bool PhaseSpace2to2tauyz::finalKin() {
  ...
  } else if ( beamAPtr->isUnresolved() && beamAPtr->isLepton() && beamBPtr->isHadron() ) {

    // Use lepton mass when DIS.
    if (settingsPtr->flag("Beams:massiveLeptonBeams")) mH[1] = mA;
    double eCM1 = 0.5 * ( s + pow2(mA) - pow2(mB) ) / eCM;
    /// double eCM2 = 0.25 * x2H * s / eCM1;
    double eCM2 = 0.5 * (x2H * s - pow2(mH[1]))
               / ( eCM1 + sqrt( pow2(eCM1) - pow2(mH[1]) ) );

    // Set initiator momenta in CM frame of beam particles.
    /// pH[1] = Vec4( 0., 0.,  eCM1, sqrt(eCM1*eCM1 + mH[1]*mH[1]) );
    pH[1] = Vec4( 0., 0.,  sqrt( pow2(eCM1) - pow2(mH[1]) ), eCM1 );
    pH[2] = Vec4( 0., 0., -eCM2, eCM2);

  } else if ( beamBPtr->isUnresolved() && beamBPtr->isLepton() && beamAPtr->isHadron() ) {

    // Use lepton mass when DIS.
    if (settingsPtr->flag("Beams:massiveLeptonBeams")) mH[2] = mB;
    double eCM2 = 0.5 * ( s - pow2(mA) + pow2(mB) ) / eCM;
    /// double eCM1 = 0.25 * x1H * s / eCM2;
    double eCM1 = 0.5 * (x1H * s - pow2(mH[2]))
               / ( eCM2 + sqrt( pow2(eCM2) - pow2(mH[2]) ) );

    // Set initiator momenta in CM frame of beam particles.
    pH[1] = Vec4( 0., 0.,  eCM1, eCM1);
    /// pH[2] = Vec4( 0., 0., -eCM2, sqrt(eCM2*eCM2 + mH[2]*mH[2]));
    pH[2] = Vec4( 0., 0., -sqrt( pow2(eCM2) - pow2(mH[2]) ), eCM2 );
  ...
  // Special kinematics for DIS to preserve lepton mass.
             ...
             || (beamBPtr->isUnresolved() && beamBPtr->isLepton() && beamAPtr->isHadron()) )
  ...
  betaZ = (x1H - x2H)/(x1H + x2H);
  phi   = 2. * M_PI * rndmPtr->flat();
  // Special case for DIS to ensure that x is preserved.
  if (      beamAPtr->isLepton() && beamBPtr->isHadron()
    && !settingsPtr->flag("PDF:lepton2gamma") ) {
    Vec4 pH1 = Vec4( 0., 0.,  0.5*sqrt(sH), 0.5*sqrt(sH) );
    //double eCM1 = 0.5 * ( sH + pow2(mA) ) / sqrt(sH);
    //Vec4 pH1 = Vec4( 0., 0., sqrt( pow2(eCM1) - pow2(mA) ), eCM1);
    MfromCM.bst( pH1, pH[1]);
  } else if ( beamBPtr->isLepton() && beamAPtr->isHadron()
    && !settingsPtr->flag("PDF:lepton2gamma") ) {
    Vec4 pH2 = Vec4( 0., 0.,  -0.5*sqrt(sH), 0.5*sqrt(sH) );
    //double eCM2 = 0.5 * ( sH + pow2(mB) ) / sqrt(sH);
    //Vec4 pH2 = Vec4( 0., 0., sqrt( pow2(eCM2) - pow2(mB) ), eCM2);
    MfromCM.bst( pH2, pH[2]);
  } else {
    MfromCM.bst( 0., 0., betaZ);
  }
  ...
  /*Vec4 pH3_store(pH[3]), pH4_store(pH[4]); 
  cout << "\nMomentum used for hard process" << endl;
  cout << "inA=" << pH[1].mCalc() << " " << pH[1]
       << "inB=" << pH[2].mCalc() << " "<< pH[2]
       << "lep=" << pH3_store.mCalc() << " " << pH3_store
       << "had=" << pH4_store.mCalc() << " " << pH4_store;
  cout << scientific << setprecision(8);
  cout << "x used for hard process       " << x1H << " " << x2H << "\t"
  << 2.*pH[1].e()/eCM << " " <<  2.*pH[2].e()/eCM << "\t"
  << pH[1].pPos()/eCM << " " <<  pH[2].pNeg()/eCM << endl;
  cout << "Difference in tHat            " << (pH[1]-pH3_store).m2Calc()-tH << endl;
  cout << "Difference in tHat            " << (pH[2]-pH4_store).m2Calc()-tH << endl;
  cout << "Difference in sHat            " << (pH[1]+pH[2]).m2Calc()-sH
                                    << " " << (pH3_store+pH4_store).m2Calc()-sH << endl;
  abort();*/
